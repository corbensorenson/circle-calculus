...
The Python sidecar also includes a bounded generator-search fixture. It ranks only a declared finite candidate list, reports how many candidates regenerate exactly, and separates the shortest exact candidate from the shortest exact candidate that is also shorter than explicit storage. The empty-search fixture records the zero-candidate boundary explicitly. The best-candidate soundness theorems record the nonempty boundary: if the bounded search returns a best exact candidate, that candidate is exact and comes from the declared finite search space. This is a scoped search report, not a proof of global minimality.
