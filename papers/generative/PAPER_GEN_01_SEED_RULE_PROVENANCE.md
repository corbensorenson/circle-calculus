...
`GEN-T0001` through `GEN-T0043` are Lean-proved finite seeds in `Circle.Generative.SeedRule`.
...
`GEN-T0035` through `GEN-T0037` strengthen the whole declared exact-candidate list: every exact candidate is inside the finite candidate search space, every exact candidate is exact, and positive exact-count is equivalent to the existence of a returned best exact candidate. `GEN-T0038` and `GEN-T0039` pin the singleton self-comparison to both the exact-candidate list and candidate list. `GEN-T0040` through `GEN-T0043` tighten finite-circle node generation: generated membership is exactly `< n`, generated nodes are duplicate-free, `n = 0` produces the empty list, and positive `n` produces a nonempty list. They still do not prove that the list is an optimal or minimal compression.
