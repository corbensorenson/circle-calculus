...
- `GEN-T0031`: `Circle.Generative.singletonExactGeneratorSearch_candidateCount`, a singleton exact generator search has exactly one declared candidate.
- `GEN-T0032`: `Circle.Generative.singletonExactGeneratorSearch_exactCandidateCount`, a singleton exact generator search has exactly one exact-regeneration candidate.
- `GEN-T0033`: `Circle.Generative.singletonExactGeneratorSearch_bestExact_some`, a singleton exact generator search returns the self-comparison as its best exact candidate.
- `GEN-T0034`: `Circle.Generative.singletonExactGeneratorSearch_bestExact_exact`, the best candidate returned by a singleton exact generator search satisfies exact regeneration.
- `GEN-T0035`: `Circle.Generative.boundedGeneratorSearch_exactCandidate_mem_candidates`, every declared exact candidate in a bounded generator search belongs to the declared finite candidate search space.
- `GEN-T0036`: `Circle.Generative.boundedGeneratorSearch_exactCandidate_exact`, every declared exact candidate in a bounded generator search satisfies exact regeneration.
- `GEN-T0037`: `Circle.Generative.boundedGeneratorSearch_exactCandidateCount_pos_iff_exists_bestExact`, a bounded generator search has positive exact-candidate count exactly when it returns some best exact candidate.
- `GEN-T0038`: `Circle.Generative.singletonExactGeneratorSearch_bestExact_mem_exactCandidates`, the singleton exact-search self-comparison is a member of its exact-candidate list.
- `GEN-T0039`: `Circle.Generative.singletonExactGeneratorSearch_bestExact_mem_candidates`, the singleton exact-search self-comparison is a member of its declared finite candidate list.
- `GEN-T0040`: `Circle.Generative.finiteCircleGenerator_generatedNodes_mem_iff_lt`, a finite-circle generator produces exactly the node addresses below `n`.
- `GEN-T0041`: `Circle.Generative.finiteCircleGenerator_generatedNodes_nodup`, a finite-circle generator produces node addresses without duplicates.
- `GEN-T0042`: `Circle.Generative.finiteCircleGenerator_generatedNodes_zero`, the zero-size finite-circle generator produces the empty node list.
- `GEN-T0043`: `Circle.Generative.finiteCircleGenerator_generatedNodes_nonempty`, a positive-size finite-circle generator produces a nonempty node list.
- `GEN-T0044`: `Circle.Generative.boundedGeneratorSearch_exactCandidateCount_le_candidateCount`, a proof-carrying bounded generator search records that exact candidates cannot outnumber declared candidates.
- `GEN-T0045`: `Circle.Generative.boundedGeneratorSearch_bestExact_some_candidateCount_pos`, a returned best exact candidate implies a positive declared candidate count.
- `GEN-T0046`: `Circle.Generative.generatorStorageComparison_shorter_iff_positive_saving`, a finite generator-storage comparison is shorter exactly when its natural-number storage saving is positive.
- `GEN-T0047`: `Circle.Generative.generatorStorageComparison_storageSaving_add_generatorLength`, a shorter finite generator-storage comparison accounts for explicit length as storage saving plus generator length.
- `GEN-T0048`: `Circle.Generative.finiteCircle128StorageComparison_generatorShorter`, the public finite-circle-128 fixture has a shorter generator description under the declared length metric.
- `GEN-T0049`: `Circle.Generative.finiteCircle128StorageComparison_storageSaving`, the public finite-circle-128 fixture has storage saving `71` under the declared length metric.
- `GEN-T0050`: `Circle.Generative.finiteCircle128StorageComparison_positive_saving`, the public finite-circle-128 fixture has positive storage saving under the declared length metric.

`GEN-T0001` through `GEN-T0050` are Lean-proved finite seeds in `Circle.Generative.SeedRule`.
`GEN-T0003` proves exact regeneration of the declared representative-indexed list model. `GEN-T0006` through `GEN-T0013` strengthen that model with count, period, coverage, formal orbit-class agreement, canonical representative coverage, and representative disjointness facts. `GEN-T0014` through `GEN-T0019` make exact regeneration reusable as a reflexive, symmetric, transitive, and field-equality-gated relation for constructed generator comparisons. `GEN-T0020` and `GEN-T0021` add direct generated-list length certificates for finite-circle nodes and single coil-orbit schedules. `GEN-T0022` through `GEN-T0024` prove the empty declared-search boundary: zero candidates, zero exact candidates, and no best exact candidate. `GEN-T0031` through `GEN-T0034` prove the positive singleton exact-search boundary: one declared candidate, one exact candidate, a returned best exact self-comparison, and exact regeneration for that returned candidate. `GEN-T0025` through `GEN-T0027` prove the nonempty soundness boundary for any returned best exact candidate: it comes from the declared exact list, it comes from the declared finite candidate list, and it is exact. `GEN-T0028` through `GEN-T0030` connect best-exact absence or presence to exact-candidate emptiness and count gates.
`GEN-T0035` through `GEN-T0037` strengthen the whole declared exact-candidate list: every exact candidate is inside the finite candidate search space, every exact candidate is exact, and positive exact-count is equivalent to the existence of a returned best exact candidate. `GEN-T0038` and `GEN-T0039` pin the singleton self-comparison to both the exact-candidate list and candidate list. `GEN-T0040` through `GEN-T0043` tighten finite-circle node generation: generated membership is exactly `< n`, generated nodes are duplicate-free, `n = 0` produces the empty list, and positive `n` produces a nonempty list. `GEN-T0044` and `GEN-T0045` add finite-search count guardrails: the exact-candidate count is bounded by the declared candidate count, and any returned best exact candidate implies that the declared search space is nonempty. `GEN-T0046` through `GEN-T0050` add finite storage-accounting for a declared comparison and the public `C_128` fixture. They still do not prove that the list is globally optimal, minimal compression, or that seed-rule storage is usually shorter.
...
The Python sidecar also includes a bounded generator-search fixture. It ranks only a declared finite candidate list, reports how many candidates regenerate exactly, and separates the shortest exact candidate from the shortest exact candidate that is also shorter than explicit storage. The empty-search fixture records the zero-candidate boundary explicitly. The best-candidate soundness theorems record the nonempty boundary: if the bounded search returns a best exact candidate, that candidate is exact, comes from the declared finite search space, and has a positive exact-candidate count. The count-guardrail theorems also require exact-candidate count to be bounded by declared candidate count and make any returned best exact candidate imply a positive declared candidate count. If no best exact candidate exists, the declared exact-candidate list and count are empty. This is a scoped search report, not a proof of global minimality.

The public Circle AI contract pack exports those bounded-search fields beside the `C_128` storage-accounting fixture so downstream tools can consume the declared finite-search scope, exact-candidate count, best-exact existence check, and best-shorter existence check without reading the sidecar code.
