...
- `AIRA-T0210`: `Circle.Applications.ropeCollisionPairCountFittingMultipleCount_spec`
- `AIRA-T0211`: `Circle.Applications.ropeCollisionPairCountAtGapMultiples_eq_fittingRange`
- `AIRA-T0212`: `Circle.Applications.ropeCollisionPairCountAtGapMultiples_twice_eq_closedForm`
- `AIRA-T0213`: `Circle.Applications.ropeCollisionPairCountAtGapMultiples_eq_closedForm`
...
- `AIRA-T0214`: `Circle.Applications.ropeTurnRatioExactWeakestGapMargin_iff_finiteMargin_and_realized`
...
- `AIRA-T0215`: `Circle.Applications.ropeRationalPreset4099_exactWeakestGapMargin_report`
- `AIRA-T0222`: `Circle.Applications.ropeTurnRatioOneOverNat_gapOneNearestIntegerMargin`
- `AIRA-T0223`: `Circle.Applications.ropeTurnRatioOneOverNat_exactWeakestGapMargin_report`
- `AIRA-T0224`: `Circle.Applications.ropeTurnRatioGapNearestIntegerMargin_le_error`
- `AIRA-T0225`: `Circle.Applications.ropeTurnRatioError_natRatio_eq_one_over_den_of_modular_inverse_witness`
- `AIRA-T0226`: `Circle.Applications.ropeTurnRatioNatRatio_exactWeakestGapMargin_report_of_modular_inverse_witness`
- `AIRA-T0227`: `Circle.Applications.ropeTurnRatioNatRatio_exists_exactWeakestGapMargin_report_of_coprime`
- `AIRA-T0228`: `Circle.Applications.ropeTurnRatioFiniteMargin_natRatio_full_denominator_iff_margin_le_one_over_den`
- `AIRA-T0229`: `Circle.Applications.ropeTurnRatioNatRatio_full_denominator_obstruction_of_one_over_den_lt_margin`
- `AIRA-T0230`: `Circle.Applications.ropeTurnRatioExactWeakestGapMargin_request_iff`
- `AIRA-T0231`: `Circle.Applications.not_ropeTurnRatioFiniteMargin_of_exactWeakestGapMargin_lt_request`
...
- `AIRA-T0209`: `Circle.Applications.ropeTurnRatioFiniteMargin_contextRange_bracket_of_obstruction`
- `AIRA-T0208`: `Circle.Applications.ropeStandardChannel0D19_contextRange_margin_bracket`
- `AIRA-T0216`: `Circle.Applications.ropeTurnRatioFiniteMargin_contextRange_request_bracket_of_obstruction`
- `AIRA-T0217`: `Circle.Applications.ropeStandardChannel0D19_contextRange_request_margin_bracket`
...
`AIRA-T0043` lifts the same finite-context margin certificate to a finite real-phase bank: one member channel with a proved finite-context margin is enough to rule out all-channel near-turn collision below the scaled tolerance. `AIRA-T0044` proves the monotonic transfer law for finite-context margins: a margin certified for a larger context automatically applies to any smaller requested context. `AIRA-T0047` proves the same downward closure for the margin value itself: a larger certified lower bound proves any smaller advertised margin over the same context. `AIRA-T0045` packages context transfer at the bank level, so one channel margin certified up to a larger horizon can rule out all-channel near-turn collision for smaller requested contexts. `AIRA-T0050` packages both transfers at once: a larger-context, larger-margin channel certificate can be reused for a smaller requested context and a smaller advertised margin in the final bank-level no-near-turn statement. `AIRA-T0054` connects the abstract finite-context predicate to the concrete generated gap range `List.range context`: checking every positive generated gap is equivalent to the predicate's gap side. `AIRA-T0058` proves the fixed-value nearest-integer bridge: checking a lower bound against the integer floor and integer ceiling is equivalent to checking the same bound against every integer turn. `AIRA-T0059` applies that bridge to every generated positive gap, making the finite-context predicate equivalent to two floor/ceiling witness checks per gap. `AIRA-T0182` packages those two endpoint errors into one scalar nearest-gap margin, and `AIRA-T0183` proves that a finite-context turn-ratio margin is equivalent to lower-bounding that scalar for every positive generated gap. `AIRA-T0214` packages the public exact-weakest-gap report shape: a reported witness gap and exact margin are correct precisely when the finite-margin lower bound holds and that lower bound is attained at the reported gap. `AIRA-T0177` closes the certificate interface itself: the concrete finite nearest-integer witness certificate object is equivalent to the abstract finite-context margin predicate, not merely a one-way proof payload. `AIRA-T0186` adds the count/witness guardrail used by no-zero reports: a strictly positive finite-margin certificate rules out exact zero turn error for every generated positive gap and every integer turn count. `AIRA-T0178` closes the negative side: failure of the finite-context margin predicate is equivalent to the existence of a positive in-context gap and integer-turn witness whose error is below the advertised margin. `AIRA-T0181` then closes the one-channel real-phase contract shape: under nonnegative frequency and positive full-turn scale, a finite turn-ratio margin is equivalent to the absence of all in-context near-turn witnesses below the scaled tolerance. `AIRA-T0196` moves that equivalence to the finite nearest-integer certificate object itself, so report payloads can cite the certificate object directly rather than first translating it to the abstract margin predicate. `AIRA-T0197` adds the corresponding bank-level report bridge: if one frequency in the declared bank has such a certificate, then the all-channel near-turn predicate is ruled out after context and margin downgrades. Together these remove the infinite integer-turn search obligation for positive certificate objects and make obstruction reports theorem-shaped, but they still do not prove that a concrete irrational or nonperiodic RoPE turn ratio has a positive lower bound. `AIRA-T0053` is the first negative guardrail for this path: an integer turn ratio has no positive finite-context margin once the context includes adjacent positions. `AIRA-T0055` extends the guardrail to natural rational ratios: if `turnRatio = numerator / denominator` and the context contains the denominator gap, that gap lands exactly on `numerator` full turns, so the channel again cannot certify any positive finite-context margin. `AIRA-T0056` gives the matching positive reduced-rational finite-context certificate: if `numerator` and `denominator` are coprime and the requested context stays at or below the denominator, then the nearest-integer margin is at least `1 / denominator`. `AIRA-T0057` packages the exact boundary: for a reduced natural rational turn ratio, that same `1 / denominator` certificate holds iff the inspected context does not exceed the denominator.

`AIRA-T0060` packages the finite witness payload as a proof-carrying certificate object, and `AIRA-T0177` proves that this object is equivalent to the abstract finite-context margin predicate. `AIRA-T0061` is the first named end-to-end finite-margin preset: the declared rational/discretized turn ratio `1/4099` has margin `1/4099` over context `4096`. `AIRA-T0185` proves the exact weakest-gap ingredients for that preset: gap `1` realizes scalar nearest-integer margin `1/4099`, and every positive generated gap in the inspected context has margin at least that value. `AIRA-T0215` packages those ingredients into the reusable exact-weakest-gap report contract. `AIRA-T0222` and `AIRA-T0223` generalize the exact-gap part of that preset: for every rational/discretized turn ratio `1/d`, every nontrivial context with `context <= d` has exact weakest scalar margin `1/d`, realized at gap `1`. `AIRA-T0224` through `AIRA-T0226` extend the exact weakest-gap report shape to arbitrary reduced natural-rational turn ratios when the certifier supplies a concrete gap whose numerator product is `+1` or `-1` from an integer number of denominators; this covers examples such as `3/7`, where gaps `2` and `5` realize exact scalar margin `1/7`. `AIRA-T0227` removes that witness-supply caveat for the full-denominator context: coprimality alone guarantees that some positive gap below the denominator realizes the exact weakest scalar margin `1/denominator`. `AIRA-T0228` turns that witness existence into the full threshold contract: at context `denominator`, the finite-margin predicate holds exactly for advertised margins `<= 1/denominator`, and larger margins fail. `AIRA-T0229` adds the report-facing failure witness: if a requested margin is above that exact threshold, Lean proves there is a positive gap below the denominator and an integer turn whose error is already below the request. `AIRA-T0230` and `AIRA-T0231` generalize that request threshold to every exact weakest-gap certificate: a requested margin holds exactly when it is at most the certified weakest margin, and every larger request fails. `AIRA-T0187` proves the adjacent no-zero report: no positive generated gap below `4096` has exact zero error against any integer turn count, matching the Python `zero_margin_witness = None` field. `AIRA-T0196` says the named finite-certificate object is equivalent to the one-channel no-near-turn contract shape, and `AIRA-T0062` applies that certificate to rule out one-channel near-turn collision below the certified margin. This is a complete theorem-backed certificate for a rational/discretized turn-ratio policy; it still does not prove the irrational/nonperiodic standard RoPE margin theorem.

`AIRA-T0063` and `AIRA-T0064` add the interval-certificate route. A rational enclosure of `gap * alpha` that lies inside one integer cell and stays at least `margin` from both cell endpoints proves the nearest-integer lower bound for that gap; a finite table of those witnesses proves the full finite-context turn-ratio margin. `AIRA-T0065` through `AIRA-T0067` instantiate that route for the genuine standard RoPE channel-0 turn ratio `alpha = 1 / (2π)` over context `6`, with margin `1/8`, using `3 < π <= 4`. `AIRA-T0068` through `AIRA-T0138` build the interval-certificate route through four-decimal, six-decimal, and 20-decimal rational `π` enclosures, the gap-`710` obstruction family, margin monotonicity, bank transfer, and standard channel-0 seeds through context `16384`. `AIRA-T0139` through `AIRA-T0141` add the rational-band compression route: a valid list of rational interval bands that covers every positive generated gap proves the same interval-certificate predicate, and a purely rational endpoint-validity predicate reflects into the real-valued band-validity predicate once the rational turn-ratio enclosure bounds the real turn ratio. `AIRA-T0142` through `AIRA-T0147` use that route for the generated D15 context-32768 certificate, one-channel no-near-turn theorem, conditional bank transfers, and 32k bracket. `AIRA-T0148` through `AIRA-T0153` do the same for the generated D16 context-65536 certificate at margin `1/104219`. `AIRA-T0154` through `AIRA-T0161` add the D17 128k certificate at margin `1/328459` plus the adjacent obstruction showing margins at or above `1/328458` are impossible once gap `103993` is in scope; `AIRA-T0162` through `AIRA-T0167` extend the same lower-margin generated certificate to context `163840`; `AIRA-T0168` through `AIRA-T0173` extend it to context `196608`; `AIRA-T0209` proves the generic context-range bracket bridge from one certified horizon plus one obstruction gap; and `AIRA-T0208` applies that bridge to package the D19 endpoint result into the reusable range contract `103993 < context <= 196608`. `AIRA-T0216` turns that range bracket into the request-level classifier used by the Python certifier: requested margins at or below the proved threshold are certified, requested margins at or above the obstruction threshold are rejected, and the open gap between those thresholds remains unclaimed. `AIRA-T0217` specializes that classifier to standard channel 0 at the D19 frontier. `AIRA-T0218` (`Circle.Applications.ropeStandardChannel0D19_request_margin_thresholds_ordered`) proves the D19 threshold ordering `1/328459 < 1/328458`, and `AIRA-T0219` (`Circle.Applications.ropeStandardChannel0D19_request_margin_branches_disjoint`) proves the report-facing consistency invariant that no requested margin can inhabit both the proved and impossible classifier branches. `AIRA-T0220` (`Circle.Applications.ropeStandardChannel0D19_request_margin_open_gap_iff_unclassified`) proves that the undecided report status is exactly the open interval between those two thresholds, and `AIRA-T0221` (`Circle.Applications.ropeStandardChannel0D19_request_margin_trichotomy`) proves the three-way margin trichotomy: every requested margin is in the proved region, the impossible region, or that open undecided region. `AIRA-T0233` (`Circle.Applications.ropeStandardChannel0D19_contextRange_request_margin_semantic_trichotomy`) proves the in-range semantic branch trichotomy, `AIRA-T0234` (`Circle.Applications.ropeStandardChannel0D19_proved_request_firstChannel_bank_noNearTurn`) transfers the proved branch to the first-channel bank shape, `AIRA-T0235` (`Circle.Applications.ropeStandardChannel0D19_proved_request_firstChannel_bank_noNearTurn_onContext`) packages that transfer as a context-wide guarantee for every ordered unequal pair inside the requested context, `AIRA-T0236` (`Circle.Applications.ropeStandardChannel0D19_proved_request_radianFirstChannel_bank_noNearTurn_onContext`) specializes the same guarantee to the ordinary radian bank form with first frequency `1` and full turn `2π`, and `AIRA-T0237` (`Circle.Applications.ropeStandardChannel0D19_contextRange_radianFirstChannel_request_semantic_trichotomy`) packages the in-range semantic trichotomy together with that radian first-channel consequence. `AIRA-T0239` (`Circle.Applications.exists_ropeTurnRatioError_le_inv_context`) adds a general Diophantine guardrail: for every turn ratio and every nontrivial finite context, some positive in-context gap has nearest-integer error at most `1/context`. `AIRA-T0240` (`Circle.Applications.not_ropeTurnRatioFiniteMargin_of_inv_context_lt_margin`) turns that witness theorem into the direct consequence that no finite-context margin strictly larger than `1/context` can hold. These theorems are upper-bound ceilings on possible finite-margin claims, not positive standard-RoPE margin certificates. The generated source stores the large D15/D16/D17/D18/D19 band tables in a compact uniform form, attaching the shared rational lower/upper turn-ratio bounds inside Lean before the same rational-band proof bridge consumes the list. The generated coverage proof now checks that those bands form one contiguous gap chain, then uses a generic Lean bridge from contiguity to ordinary band coverage; endpoint validity is checked on compact uniform rows and reflected into expanded band validity by a shared Lean bridge. This is real standard-RoPE theorem infrastructure, but it is intentionally still channel-0 based: it does not prove margins for every channel in a multi-channel bank.
...
- fitting common-gap multiple count when exact discrete distinguishability fails;
