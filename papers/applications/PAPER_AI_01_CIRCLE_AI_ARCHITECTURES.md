...
- Use the private Theseus-Hive consumers for `site/data/generated/theseus_hive_ai_contracts.json` as the report boundary, keeping private reports out of the public Circle repository unless they are intentionally scrubbed and public-safe; next use the full-frontier fanout, recurrence, memory, and phase tables to design Circle selection, loop-depth, downstream retrieval, or route/ranker workloads that differ from ordinary baselines when evidence supports it, and promote the remaining mixer and seed-rule aggregate attachments into scored private benchmarks.
