...
- Use the private Theseus-Hive consumers for `site/data/generated/theseus_hive_ai_contracts.json` as the report boundary, keeping private reports out of the public Circle repository unless they are intentionally scrubbed and public-safe; next use the full-frontier fanout and recurrence failure/runtime tables to design Circle selection or loop-depth workloads that differ from ordinary rank-one or fixed-depth baselines when evidence supports it, and promote the remaining memory, phase-feature, mixer, and seed-rule aggregate attachments into scored private benchmarks.
