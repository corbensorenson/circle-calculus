...
The active Theseus-Hive trainer itself now has opt-in pure and residual Circle mixer shadow paths in `scripts/train_octopus_router_head.py`: `--circle-mixer circulant`, `--circle-mixer block_cyclic`, `--circle-mixer circulant_residual`, and `--circle-mixer block_cyclic_residual`. The default no-mixer sparse centroid head is unchanged. The pure modes hash sparse features into finite channels; the residual modes keep raw features and add cyclic channels beside them. The aggregate shadow benchmark `scripts/circle_router_head_mixer_shadow_benchmark.py` emits `circle_router_head_mixer_shadow_training_v1` with private-safe metrics and hashed failure summaries. A private-safe Mac shadow run over the current local ORA report, arm registry, and workflow routing traces found baseline, pure circulant, pure block-cyclic, residual circulant, and residual block-cyclic all tied at `14`/`14` holdout exact-set accuracy, arm micro-F1 `1.0`, risk-routing accuracy `1.0`, exact mismatch count `0`, and zero external inference calls. A `5`-repetition timing/memory run recorded stable aggregate metrics; validation-run median elapsed time was `8.926` ms for baseline, `105.975` ms for pure circulant, `110.399` ms for pure block-cyclic, `104.434` ms for residual circulant, and `112.164` ms for residual block-cyclic, and median Python `tracemalloc` peak bytes were `117078`, `335105`, `338973`, `447449`, and `438130`, respectively. The new private-safe keyword-boundary diagnostic has `45` holdout examples: no-mixer exact accuracy is `1.0`, pure circulant and pure block-cyclic are `0.6667`, and full-gain residual circulant/residual block-cyclic improve to `0.8667`/`0.8222` while still trailing baseline. A quality-only gain sweep finds residual gains `0.05`, `0.1`, `0.25`, and `0.5` preserve baseline exactness on this synthetic diagnostic, while full gain `1.0` regresses. This is real trainer-path compatibility, regression, and tuning evidence, not a claim that Circle structure improves Theseus-Hive quality, speed, memory, or transfer.
