...
- Use the private Theseus-Hive consumers for `site/data/generated/theseus_hive_ai_contracts.json` as the report boundary, keeping private reports out of the public Circle repository unless they are intentionally scrubbed and public-safe; next use the full-frontier failure/runtime table to design a Circle selection rule that differs from ordinary rank-one when evidence supports it, and promote the other five aggregate attachments into scored private recurrence, memory, phase-feature, mixer, and seed-rule benchmarks.
