...
The local companion Theseus-Hive workspace now has a report-only consumer, `scripts/circle_ai_contract_consumer.py`, a structural smoke workload layer, `scripts/circle_ai_private_workload_smoke.py`, a deterministic proxy benchmark layer, `scripts/circle_ai_private_proxy_benchmark.py`, a real local aggregate attachment layer, `scripts/circle_ai_real_workload_attachments.py`, and a first scored private benchmark layer, `scripts/circle_ai_scored_private_benchmarks.py`, with a matching note at `docs/CIRCLE_CALCULUS_TRANSFER.md`. The consumer loads the Circle pack, writes `reports/circle_ai_contract_consumer.json` and `.md`, separates quality/runtime/memory/parameter/interpretability/transfer/failure-case axes, and keeps its state `YELLOW` until named private benchmark results exist. The smoke layer adds six stable workload slots: `circle_recurrence_budget_trace_smoke`, `circle_strided_candidate_coverage_smoke`, `circle_memory_alias_visibility_smoke`, `circle_phase_feature_invariance_smoke`, `circle_mixer_parameter_accounting_smoke`, and `circle_seed_rule_regeneration_smoke`. The proxy layer adds measured deterministic baseline metrics under `circle_recurrence_budget_proxy_v1`, `circle_candidate_fanout_proxy_v1`, `circle_memory_alias_proxy_v1`, `circle_phase_feature_proxy_v1`, `circle_mixer_parameter_proxy_v1`, and `circle_seed_rule_regeneration_proxy_v1`. The aggregate attachment layer adds `circle_recurrence_budget_real_rows_v1`, `circle_candidate_fanout_real_manifest_v1`, `circle_memory_alias_real_trace_v1`, `circle_phase_feature_real_sequence_v1`, `circle_mixer_real_report_summary_v1`, and `circle_seed_rule_real_provenance_summary_v1`. The scored layer adds `circle_candidate_fanout_scored_manifest_v1`, `circle_candidate_fanout_semantic_frontier_v1`, `circle_candidate_fanout_equal_budget_semantic_v1`, `circle_recurrence_work_budget_semantic_v1`, `circle_memory_context_packet_retrieval_v1`, `circle_phase_feature_sequence_ablation_v1`, `circle_mixer_route_ranker_ablation_v1`, `circle_mixer_router_head_attachment_v1`, `circle_seed_rule_exact_regeneration_v1`, `circle_seed_rule_source_change_stress_v1`, `circle_seed_rule_workflow_tool_card_rebuild_v1`, and `circle_seed_rule_update_cycle_effort_v1`, comparing Circle and ordinary candidate selections on private manifest score/gate fields, existing private semantic score-report context, a full-frontier private semantic execution run, a recurrence work-budget execution run over 1040 tasks across 26 categories, context-packet identity/retention policies over 37 ledger targets, phase-feature ablations over 9750 private sequence/category rows, route/ranker mixer transforms over 94 local Octopus router traces, router-head mixer attachment over 14 current Octopus holdout decisions, seed-rule exact regeneration over 75 safe provenance-envelope records, seed-rule source-change stress over 4 deterministic scenarios with 20 downstream reuse queries, workflow/tool-card rebuild over 72 safe workflow/card sources and 188 safe envelope records, and update-cycle effort instrumentation over 289 safe aggregate event records with 212 runtime fields, three review-step fields, and paired object-only/Circle maintenance-mode labels. The private side now also has `scripts/hive_record_maintenance_effort.py` to record measured human-edit rows for future update-cycle comparisons. It still marks learned-model quality metrics, promotion evidence, private data export, and measured human-edit rows absent; the full-frontier scored runs tie the relevant ordinary rank-one or fixed-depth baselines, equal-budget memory retention ties ordinary retention baselines, phase features tie no-phase/wrong-period/position-bucket controls, the mixer fixture ties dense heldout trace accuracy with lower counted parameters, the router-head attachment ties current holdout-decision agreement with the same lower counted Circle parameter budget, the seed-rule fixture exactly regenerates safe provenance envelopes with zero verifier residuals and a shorter source/rule recipe than object-only storage, the seed-rule stress fixture ties object-only exactness/reuse while recording lower average description length and edit cost, the workflow rebuild fixture ties object-only exactness/reuse while recording lower automated rebuild work units, and the update-cycle effort instrumentor still finds no human-edit-minute fields rather than proving deployed-router, model-quality, human-time, maintenance-savings, or universal-compression improvement.

The active Theseus-Hive trainer itself now has opt-in pure and residual Circle mixer shadow paths in `scripts/train_octopus_router_head.py`: `--circle-mixer circulant`, `--circle-mixer block_cyclic`, `--circle-mixer circulant_residual`, and `--circle-mixer block_cyclic_residual`. The default no-mixer sparse centroid head is unchanged. The pure modes hash sparse features into finite channels; the residual modes keep raw features and add cyclic channels beside them. The aggregate shadow benchmark `scripts/circle_router_head_mixer_shadow_benchmark.py` emits `circle_router_head_mixer_shadow_training_v1` with private-safe metrics and hashed failure summaries. A private-safe Mac shadow run over the current local ORA report, arm registry, and workflow routing traces found baseline, pure circulant, pure block-cyclic, residual circulant, and residual block-cyclic all tied at `14`/`14` holdout exact-set accuracy, arm micro-F1 `1.0`, risk-routing accuracy `1.0`, exact mismatch count `0`, and zero external inference calls. A `5`-repetition timing/memory run recorded stable aggregate metrics; validation-run median elapsed time was `10.704` ms for baseline, `112.769` ms for pure circulant, `96.332` ms for pure block-cyclic, `100.91` ms for residual circulant, and `97.362` ms for residual block-cyclic, and median Python `tracemalloc` peak bytes were `117078`, `335297`, `339165`, `447689`, and `438394`, respectively. The new private-safe keyword-boundary diagnostic has `45` holdout examples: no-mixer exact accuracy is `1.0`, pure circulant and pure block-cyclic are `0.6667`, and full-gain residual circulant/residual block-cyclic improve to `0.8667`/`0.8222` while still trailing baseline. A quality-only synthetic gain sweep finds residual gains `0.05`, `0.1`, `0.25`, and `0.5` preserve baseline exactness on this synthetic diagnostic, while full gain `1.0` regresses. The real-trace boundary split then uses `30` private workflow routing rows, `5` eligible private labelsets, `71` train examples, and `9` holdout examples: full-gain Circle modes score `0.8889` exact accuracy against the no-mixer `1.0`, while residual gains `0.05`, `0.1`, and `0.25` preserve baseline exactness and `0.5`/`1.0` regress. The companion private-safe boundary coverage report sees `31` compatible real rows but also `7` singleton hashed labelset bins, `11` under-target bins, and a `40`-row gap to bring every bin to the target of `5` rows. This is real trainer-path compatibility, regression, tuning, and coverage evidence, not a claim that Circle structure improves Theseus-Hive quality, speed, memory, or transfer.
...
- Use the private Theseus-Hive consumers for `site/data/generated/theseus_hive_ai_contracts.json` as the report boundary, keeping private reports out of the public Circle repository unless they are intentionally scrubbed and public-safe; next use the full-frontier fanout, recurrence, memory, phase, mixer, router-head attachment, and seed-rule tables to design Circle selection, loop-depth, downstream retrieval, route/ranker training, parameter-efficiency, or explicit human-effort field workloads that differ from ordinary baselines when evidence supports it.
