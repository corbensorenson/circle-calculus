...
The active Theseus-Hive trainer itself now has an opt-in Circle mixer shadow path in `scripts/train_octopus_router_head.py`: `--circle-mixer circulant` and `--circle-mixer block_cyclic`. The default no-mixer sparse centroid head is unchanged. The Circle modes hash sparse features into finite channels, apply a shared cyclic kernel, and record their configuration in the emitted model/report. The aggregate shadow benchmark `scripts/circle_router_head_mixer_shadow_benchmark.py` emits `circle_router_head_mixer_shadow_training_v1` with private-safe metrics and hashed failure summaries. A private-safe Mac shadow run over the current local ORA report, arm registry, and workflow routing traces found baseline, circulant, and block-cyclic all tied at `14`/`14` holdout exact-set accuracy, arm micro-F1 `1.0`, risk-routing accuracy `1.0`, exact mismatch count `0`, and zero external inference calls. A `5`-repetition timing/memory run recorded stable aggregate metrics; validation-run median elapsed time was `9.97` ms for baseline, `88.042` ms for circulant, and `93.043` ms for block-cyclic, and median Python `tracemalloc` peak bytes were `117078`, `335105`, and `338973`, respectively. This is real trainer-path compatibility evidence, not a claim that Circle structure improves Theseus-Hive quality, speed, memory, or transfer.
