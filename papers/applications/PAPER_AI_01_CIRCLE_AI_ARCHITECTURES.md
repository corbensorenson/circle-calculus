...
The active Theseus-Hive trainer itself now has an opt-in Circle mixer shadow path in `scripts/train_octopus_router_head.py`: `--circle-mixer circulant` and `--circle-mixer block_cyclic`. The default no-mixer sparse centroid head is unchanged. The Circle modes hash sparse features into finite channels, apply a shared cyclic kernel, and record their configuration in the emitted model/report. A private-safe Mac shadow run over the current local ORA report, arm registry, and workflow routing traces found baseline, circulant, and block-cyclic all tied at `14`/`14` holdout exact-set accuracy with risk-routing accuracy `1.0` and zero external inference calls. This is real trainer-path compatibility evidence, not a claim that Circle structure improves Theseus-Hive quality, speed, memory, or transfer.
