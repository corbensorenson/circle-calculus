...
- Use the private Theseus-Hive consumers for `site/data/generated/theseus_hive_ai_contracts.json` as the report boundary, keeping private reports out of the public Circle repository unless they are intentionally scrubbed and public-safe; next use the full-frontier fanout, recurrence, memory, phase, mixer, and seed-rule tables to design Circle selection, loop-depth, downstream retrieval, route/ranker, parameter-efficiency, or source-change/downstream-reuse workloads that differ from ordinary baselines when evidence supports it.
