...
`AIT-T0126`, `AIT-T0127`, and `AIT-T0128` make the no-zero check exact. A generated stride residue collapses to zero exactly when the finite coil period divides the step count. For singleton stride families in nonzero contexts the no-zero structural condition is equivalent to the simple threshold `pathLength < period`; for finite families it is equivalent to that same threshold holding for every admitted stride. `AIT-T0129` and `AIT-T0130` package the period-threshold check as the planner-facing lag-side and query-side shortfall/gap equivalence. `AIT-T0131` backs the new `no_zero_period_violation_witness_*` fields: when a threshold fails, the constructed witness uses `step = period`, and Lean proves that such witnesses exist exactly in the period-at-most-path-budget case. `AIT-T0132` and `AIT-T0133` back the companion `*_matches_no_zero_failure` fields, so failure status is not inferred only by Python convention.
