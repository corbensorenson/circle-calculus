...
`AIT-T0126` and `AIT-T0127` make the one-stride no-zero check exact. A generated stride residue collapses to zero exactly when the finite coil period divides the step count, so for singleton stride families in nonzero contexts the no-zero structural condition is equivalent to the simple threshold `pathLength < period`.
