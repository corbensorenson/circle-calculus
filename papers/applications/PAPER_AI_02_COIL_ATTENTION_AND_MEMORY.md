...
- `AIT-T0143` and `AIT-T0144`: the same public `4096` and `8192` rows now have
  ...
  `96`.
- `AIT-T0145` and `AIT-T0146`: the sidecar's new lag-side and query-side
  dedup-loss fields are theorem-backed at the zero boundary. Loss is zero
  exactly when the corresponding raw candidate list has no duplicates. The
  compact `C_16`, local-window `2`, path-length `4`, strides `(4,8)` probe
  demonstrates the nonzero case with duplicate generated residues.
