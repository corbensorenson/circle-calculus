...
The planner-style rows extend that same certificate shape to larger declared layouts without adding a new theorem claim. The singleton `context = 12`, `local_window = 1`, `path_length = 2`, stride `(4)` row exposes the period-threshold field: the stride period is `3`, so `2 < 3` certifies the no-zero residue condition for that one-stride plan. For `context = 4096`, `local_window = 32`, `path_length = 4`, and strides `(33,160,800)`, the report records `44` candidates per query, `44` covered lags, `4051` uncovered lags, `12` uncovered intervals, and raw-budget survival on both the lag and query sides. For `context = 8192`, `local_window = 64`, `path_length = 8`, and strides `(127,509,1021,2039)`, it records `96` candidates per query, `96` covered lags, `8095` uncovered lags, `32` uncovered intervals, and the same raw-budget survival flags. In all rows, the raw budget and unique lag-candidate count are far below `context - 1`, and the generated candidates satisfy the positive in-context range hypothesis, so `AIT-T0110` through `AIT-T0114` certify incompleteness and the exact gap count before inspecting individual lag witnesses, while `AIT-T0120` certifies that the unique-count shortfall is equivalent to such a witness existing. These rows satisfy the broader no-zero-residue sufficient condition, so `AIT-T0117` through `AIT-T0119` explain why the candidate-range iff applies from structural plan checks, `AIT-T0122` explains why the shortfall/gap iff applies structurally, and `AIT-T0128` explains why the finite-family period-threshold scan is equivalent to the no-zero structural predicate. The singleton row additionally cites `AIT-T0126` and `AIT-T0127` for the one-stride period-threshold explanation; the `4096` row also satisfies the stricter no-wrap separated sufficient condition, so `AIT-T0115`, `AIT-T0116`, and `AIT-T0121` give a stronger no-wrap reason for the same endpoints; the `8192` row does not satisfy that stricter no-wrap structural discharge. These are examples of how an engineer should read the contract: the budget may be exactly certified while coverage remains deliberately partial.
