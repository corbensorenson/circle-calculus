...
- `PHYS-T0060`: `Circle.Physics.closedGaugeLoop_fromThreeCycle_rotate_twice_holonomy`, rotating the basepoint order twice for a finite three-path closed cycle preserves packaged loop holonomy.
- `PHYS-T0061`: `Circle.Physics.closedGaugeLoop_fromThreeCycle_rotate_twice_gaugeShiftedHolonomy`, rotating the basepoint order twice for a finite three-path closed cycle preserves sampled gauge-shifted holonomy.
- `PHYS-T0062`: `Circle.Physics.closedGaugeLoop_fromThreeCycle_all_rotations_holonomy`, all cyclic basepoint rotations of a finite three-path closed cycle preserve packaged loop holonomy.
- `PHYS-T0063`: `Circle.Physics.closedGaugeLoop_fromThreeCycle_all_rotations_gaugeShiftedHolonomy`, all cyclic basepoint rotations of a finite three-path closed cycle preserve sampled gauge-shifted holonomy.
- `PHYS-T0064`: `Circle.Physics.closedGaugeLoop_gaugeShiftedHolonomy_gauge_independent`, a finite closed-loop record has the same sampled gauge-shifted holonomy for any two vertex gauge assignments.

These theorem ids are Lean-proved in `Circle.Physics.LatticeGauge` for the finite `ZMod n` phase model. `PHYS-T0006` through `PHYS-T0064` move from bare phase lists toward finite link/path records, source-target composability, singleton concatenation, three-/four-link source-target laws, empty-path identity-style composability laws, record-level identity/associativity laws for concatenation, record-level reversal laws, path-plus-reverse zero-holonomy gates, general boundary-checked append composability, first source/target laws for nonempty concatenations, a checked finite path interface with explicit endpoints, identities, associative boundary-checked composition, checked-path holonomy identity/singleton/concat laws, projection bridges back to the link-path carrier, closed checked-path gauge-invariance facts, a packaged two-path cycle gauge-invariance certificate, and a reusable closed-loop record with identity/cycle holonomy, gauge-shifted holonomy, gauge-sample independence, two-path basepoint-swap facts, and complete cyclic three-path basepoint-rotation packages. They do not yet install a full mathlib category instance for finite graph paths. They do not prove continuum gauge theory.
