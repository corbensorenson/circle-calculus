...
- Outputs: structured lag sample, nonstructured control lag sample, inspected target and candidate list, stride-family/single-stride/local/wrong-family/full rows, structured hit rates, control hit rates, average candidate counts, covered/uncovered lag counts, uncovered-lag interval summary, theorem-side lag and query-candidate counts, coil-residue no-collision, local/coil disjointness, predecessor-map injectivity, `window < context` predecessor-injectivity sufficient-condition status, no-collision flags, deduplicated and raw budget upper bounds, raw-budget and unique-lag shortfall/incompleteness guards, theorem links, and dictionary links.
- Theorem ids: `AIT-T0015` through `AIT-T0111`.
