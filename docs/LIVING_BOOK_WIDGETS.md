...
- Outputs: generated portfolio-route readiness, generated route reviewer-dossier readiness, generated portfolio backing status, role-backed value-proposition status, advertising-language guardrail status, portfolio roles, proof provenance coverage, unique and total evidence counts, route audience/claim/boundary rows, route backing rows, route dossier-section rows, capability lane, area, paper count, theorem count, executable count, Living Book page count, Living Book widget count, and the explicit claim boundary.
...
Companion non-widget prototype: `AIM-B0015` uses `circle_math.applications.circle_ai.run_tiny_looped_recurrent_prototype` with theorem ids `AIM-T0049` through `AIM-T0053`. It records certified-budget state reads, state-to-label lookup, one-step, scalar, wrong-period, and nonperiodic controls. It is documented in the AI chapter and parity-checked in `scripts/site/check_widget_python_parity.py`, but it is not a browser widget yet.
