...
The scored benchmark layer currently implements ten ids:
...
The scored layer is stronger but still limited benchmark evidence: it has named private workloads, baselines, metrics, scripts, reports, imported semantic score context, fresh equal-budget private semantic execution, fresh recurrence work-budget execution, context-packet memory retrieval scoring, phase-feature ablation scoring, route/ranker mixer ablation scoring, seed-rule exact-regeneration scoring, seed-rule source-change stress scoring, and seed-rule workflow/tool-card rebuild scoring. It still does not show a Circle advantage over the relevant ordinary quality baselines, although the mixer fixture records a parameter-count reduction at tied heldout trace accuracy and the seed-rule fixtures record shorter verified regeneration/edit/rebuild recipes than object-only storage.
...
7. Instrument real Theseus-Hive update cycles to measure human edit minutes or review steps for object-only tool-card maintenance versus Circle seed-rule rebuilds.
