...
The scored benchmark layer currently implements seven ids:
...
The scored layer is stronger but still limited benchmark evidence: it has named private workloads, baselines, metrics, scripts, reports, imported semantic score context, fresh equal-budget private semantic execution, fresh recurrence work-budget execution, context-packet memory retrieval scoring, phase-feature ablation scoring, and route/ranker mixer ablation scoring. It still does not show a Circle advantage over the relevant ordinary quality baselines, although the mixer fixture now records a parameter-count reduction at tied heldout trace accuracy.
