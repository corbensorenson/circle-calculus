...
The next trainer-shadow step is now implemented on the Theseus-Hive side. `scripts/train_octopus_router_head.py` accepts `--circle-mixer circulant` and `--circle-mixer block_cyclic`, hashes sparse router-head features into a finite channel space, applies a shared cyclic kernel, and records `feature_mixer` plus `mixer_metrics` in the emitted model/report while leaving the default no-mixer path unchanged. `scripts/circle_router_head_mixer_shadow_benchmark.py` now emits the aggregate private-safe benchmark id `circle_router_head_mixer_shadow_training_v1` without exporting task text, feature names, arm labels, or model weights. A private-safe Mac shadow run over the current local ORA report, arm registry, and workflow routing traces scored `14` holdout cases: baseline, circulant, and block-cyclic all reported exact-set accuracy `1.0`, risk-routing accuracy `1.0`, arm micro-F1 `1.0`, exact mismatch count `0`, and `external_inference_calls = 0`; both Circle modes used `4` shared kernel parameters in this trainer path. The repeated timing/memory run used `5` repetitions per mode and all aggregate metric hashes were stable. Validation-run median elapsed times were `9.97` ms for baseline, `88.042` ms for circulant, and `93.043` ms for block-cyclic; median Python `tracemalloc` peak bytes were `117078`, `335105`, and `338973`, respectively. This is actual router-head training/evaluation compatibility evidence with diagnostic runtime and memory measurements, but still not a claim of better quality, runtime, memory, transfer, or promotion readiness.
...
6. Extend the actual Octopus router-head `--circle-mixer` trainer path with harder contrastive routing traces, implementation profiling, and failure-case tables against the current no-mixer head.
