...
The real-phase theorem spine currently runs from `AIRA-T0029` through `AIRA-T0033`, then `AIRA-T0037` through `AIRA-T0045`, plus `AIRA-T0047`, `AIRA-T0050`, and `AIRA-T0053` through `AIRA-T0141`.
...
44. Use `AIRA-T0139` through `AIRA-T0141` as the rational-band compression and rational-endpoint reflection bridge for future 32k/64k generated certificates.
...
47. Replace the naive generated `interval_cases` validity proof with generated rational endpoint-validity facts that feed `AIRA-T0141` before retrying 32k/64k Lean certificates.
