...
The real-phase theorem spine currently runs from `AIRA-T0029` through `AIRA-T0033`, then `AIRA-T0037` through `AIRA-T0045`, plus `AIRA-T0047`, `AIRA-T0050`, and `AIRA-T0053` through `AIRA-T0126`.
...
`AIRA-T0065` through `AIRA-T0067` are the first named standard-RoPE interval seed. For channel 0, where `alpha = 1 / (2π)`, Lean proves a context-6 margin `1/8` using the elementary bounds `3 < π <= 4`. `AIRA-T0068` through `AIRA-T0070` strengthen the same genuine standard channel to context 7 with margin `1/32`: the witness table encloses each inspected value as `gap/8 <= gap/(2π) <= 25*gap/157` for gaps `1` through `6`, using `π <= 4` and the sharper bound `3.14 < π`. `AIRA-T0071` through `AIRA-T0073` extend the seed to context 8 by certifying gap `7` in integer cell `1` with `10/9 <= 7/(2π) <= 175/157`, using `π < 3.15` and `3.14 < π`. `AIRA-T0074` through `AIRA-T0076` extend the same margin to context 19: gaps `7` through `12` are certified in cell `1`, and gaps `13` through `18` are certified in cell `2`, with `10*gap/63 <= gap/(2π) <= 25*gap/157`. `AIRA-T0077` and `AIRA-T0078` prove the interval-witness and interval-certificate margin-monotonicity bridge. `AIRA-T0079` through `AIRA-T0081` use that bridge plus new cell bands to certify context 44 with margin `1/64`. `AIRA-T0082` and `AIRA-T0083` add four-decimal rational bounds for the same standard channel, `AIRA-T0084` through `AIRA-T0086` use those sharper bounds to cross the gap-44 near-integer obstruction and certify context 57 with margin `1/512`, `AIRA-T0087` through `AIRA-T0089` convert the generated d4 band table into a context-333, margin-`1/512` interval certificate plus no-near-turn consequence, and `AIRA-T0090` through `AIRA-T0094` add six-decimal rational bounds and certify context 710 with margin `1/1024`. `AIRA-T0095` and `AIRA-T0096` prove the matching boundary: gap `710` is already within margin `1/1024` of integer turn `113`, so the same margin cannot extend to any larger context. `AIRA-T0097` through `AIRA-T0101` add 20-decimal rational bounds and certify context 4096 with conservative margin `1/131072`. `AIRA-T0102` specializes the D9 bank-level transfer theorem, and `AIRA-T0103` through `AIRA-T0104` add the sharper D9 companion obstruction: gap `710` is within margin `1/65536` of integer turn `113`, so the doubled D9 margin cannot hold for any context containing that gap. `AIRA-T0105` through `AIRA-T0108` tighten the same context-4096 standard channel-0 seed to margin `1/105000` and carry it through the one-channel and bank-level no-near-turn consequences. `AIRA-T0109` and `AIRA-T0110` prove the nearby D10 boundary: gap `710` is within margin `1/104000` of integer turn `113`, so that larger D10 margin cannot hold for any context containing the gap. `AIRA-T0111` through `AIRA-T0114` tighten the same context-4096 seed again to margin `1/104219` and carry it through the one-channel and bank-level no-near-turn consequences. `AIRA-T0115` and `AIRA-T0116` prove the adjacent D11 boundary: gap `710` is within margin `1/104218`, so that next larger margin cannot hold for any context containing the gap. `AIRA-T0117` packages the D11 bridge for banks whose first frequency is the standard channel-0 frequency. `AIRA-T0118` generalizes the adjacent stop theorem to every advertised margin at or above `1/104218`, and `AIRA-T0119` packages the current context-4096 bracket: `1/104219` is proved, while all margins at or above `1/104218` are impossible for standard channel 0. `AIRA-T0120` through `AIRA-T0122` lower the margin to `1/104220` and extend the same d20 interval-certificate method to context 8192, `AIRA-T0123` through `AIRA-T0124` carry that longer D12 seed through the conditional one-separating-channel bank transfer, and `AIRA-T0125` packages the context-8192 bracket: `1/104220` is proved, while all margins at or above `1/104218` are impossible. `AIRA-T0126` adds a reusable endpoint-band bridge: a generated band can certify every gap inside it from monotone lower/upper turn-ratio bounds and two endpoint cell inequalities. This is theorem-backed standard-RoPE infrastructure, but still channel-0 based. It does not certify 128k or prove margins for every channel in a multi-channel standard-RoPE bank.

The Python sidecar still emits exact-rational interval plans for audit and future work. The four-decimal `π` plan through context `333`, the six-decimal `π` plan through context `710`, the conservative 20-decimal `π` plan through context `4096` at margin `1/131072`, the tighter 20-decimal plan at margin `1/105000`, the sharp 20-decimal 4k plan at margin `1/104219`, and the 20-decimal 8k plan at margin `1/104220` have now been converted into Lean declarations. Future generated plans are deterministic source data only until matching declarations compile and manifest ids are marked proved.
...
37. Use `AIRA-T0118` and `AIRA-T0119` as the current engineering-facing bracket: context `4096`, proved margin `1/104219`, and no possible advertised margin at or above `1/104218`.
...
41. Scale from one channel to broader channel-wise/full-bank statements, or add sharper generated/continued-fraction machinery for larger contexts.
