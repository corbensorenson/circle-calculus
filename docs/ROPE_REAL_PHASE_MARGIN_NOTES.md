...
The generated-proof route now compiles for D15 32k, D16 64k, and D17 128k by emitting compact band-list coverage and rational endpoint-validity facts that feed `AIRA-T0139` through `AIRA-T0141`. The generated Lean source also uses a compact uniform-band representation: each large-table row stores only `startGap`, `endGap`, and `cell`, and `ropeTurnRatioUniformRationalIntervalBands` attaches the shared 20-decimal rational lower/upper turn-ratio bounds before the existing rational-band bridge consumes the list. Coverage is now proved by `ropeTurnRatioRationalIntervalBandsContiguousCover` plus a generic bridge to ordinary band coverage, so the generated proof no longer searches every gap against every band. This reduces repeated generated source text and generated coverage proof-search weight without changing any theorem statement. The remaining proof-engineering target is sharper channel-wise certificates, full-bank standard-RoPE transfer, faster generated endpoint checking, or a continued-fraction bridge.
...
49. Use `standard_band_certificate_audits` to check that generated band lists are positive, contiguous from gap `1`, endpoint-valid, and sufficient for the requested context before attempting a generated Lean certificate. The generated Lean coverage proof now mirrors the contiguity side of that audit.
50. Treat the stronger 128k margin-`1/104219` interval plan as a failed frontier comparison, not a proved theorem id.
51. Scale from one channel to broader channel-wise/full-bank statements, optimize generated endpoint proof build time, or add sharper generated/continued-fraction machinery for larger contexts.
