...
- Learned-model work should compare against standard dense, learned-positional, RoPE, LoRA, sliding-window, dilated, full-attention, long-convolution, and state-space baselines when those baselines match the question.
...
- These are finite indexing primitives for AI prototypes plus starter phase-channel, backend-parity, cyclic-memory, coil-retrieval, adapter-block, and MultiCoil positional harnesses, not model-quality, parameter-efficiency, attention-replacement, RoPE, retrieval, or runtime claims.
...
1. Phase-channel learned-task benchmarks built from `AIA-B0001` and `AIA-B0002`.
   Use `AIA-B0003` as backend parity scaffolding while expanding the deterministic fixtures from constant/scalar-threshold baselines to learned dense, RoPE, and cyclic-feature baselines before making model-quality claims.
