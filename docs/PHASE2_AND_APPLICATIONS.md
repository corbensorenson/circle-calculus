...
6. Theseus-Hive transfer.
   Use `docs/THESEUS_HIVE_AI_TRANSFER.md`, `P5-EDGE-011`, and `site/data/generated/theseus_hive_ai_contracts.json` as the bridge from Circle AI to the user's active AI system. The public Circle-side contract fixtures now cover recurrence schedules, strided fanout coverage, cyclic memory, phase tags, circulant/block-cyclic mixers, and seed-rule provenance. The companion Theseus-Hive workspace now has report-only consumers that load those fixtures and emit YELLOW private-planning, structural-smoke, deterministic-proxy, aggregate real-workload attachment, scored candidate-manifest, semantic score-context, and full-frontier equal-budget semantic reports with zero external inference, no training mutation, no promotion evidence, six named smoke workload slots, six proxy benchmark ids, six aggregate local-artifact attachment ids, and three scored fanout ids: `circle_candidate_fanout_scored_manifest_v1`, `circle_candidate_fanout_semantic_frontier_v1`, and `circle_candidate_fanout_equal_budget_semantic_v1`. The full-frontier semantic run scores 1040 private tasks across 26 categories but ties the sequential rank-one baseline, so it is not a Circle advantage claim; next design a selection rule that differs from ordinary rank-one when evidence supports it and promote the remaining aggregate attachments into scored private workloads inside Theseus-Hive against existing position-bucket, work-budget, context-retention, dense/low-rank mixer, and object-storage baselines before claiming usefulness.
