...
- Looped/recursive transformer work starts with `AIM-B0003`, a deterministic recurrence-schedule fixture, and `AIM-B0005`, a deterministic token-level recurrence routing fixture. It should expand toward dense transformer depth, Universal Transformer recurrence, fixed looped transformers, adaptive early-exit models, recurrent-memory transformers, token-level Mixture-of-Recursions, middle-block recurrence, multi-resolution recurrence, training-free loop wrappers, sparse/MoE looped models, RWKV/Mamba-style recurrent/state-space models, and ordinary nonrecursive transformer baselines before any reasoning-depth, quality, context-length, or parameter-efficiency claim.
...
The first implementation target is `P5-EDGE-010`, now seeded by `AIM-B0003` and `AIM-B0005`: deterministic recurrence-schedule and token-level recurrence routing fixtures. They report loop budget, exit step, active-token counts, selected middle block, resolution labels, wrong/over-loop controls, and baseline labels before any MLX model training begins. Lean theorem targets should stay at finite schedule/index/closure facts unless a stronger formal model is explicitly built.
...
- These are finite indexing primitives for AI prototypes plus starter phase-channel, learned-feature baseline, harmonic-feature baseline, backend-parity, cyclic-memory, coil-retrieval, adapter-block, MultiCoil positional, and relative phase harnesses, not model-quality, parameter-efficiency, attention-replacement, RoPE, retrieval, or runtime claims.
