...
3. CoilRA adapters for small language-model fine-tuning.
   Start from `AIRA-B0001` as the adapter-block scaffold, `AIRA-B0004` as the parameter-budget scaffold, and `AIRA-B0007` as the block-cyclic mixer dense-parity and alias/load scaffold only. Compare against dense adapters, LoRA, block-circulant, circulant, and learned dense-mixer baselines. Measure quality, parameter count, inference cost, memory, failure cases, and MLX runtime on this Mac.
