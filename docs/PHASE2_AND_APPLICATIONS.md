...
- `PAPER_AI_03_COILRA_AND_MULTICOIL_ROPE` proves the adapter-block facts `AIRA-T0001` through `AIRA-T0005` in Lean through `Circle.Applications.adapterBlock_lt_blockSize`, `Circle.Applications.adapterBlock_add_blockSize`, `Circle.Applications.adapterBlock_zero`, `Circle.Applications.adapterBlock_add_mul_blockSize`, and `Circle.Applications.adapterBlock_idempotent`, proves the residue/winding position facts `AIRA-T0006` through `AIRA-T0010` in Lean through `Circle.Applications.positionResidue_lt_period`, `Circle.Applications.positionResidue_add_period`, `Circle.Applications.positionResidue_add_mul_period`, `Circle.Applications.positionWinding_mul_add_residue`, and `Circle.Applications.windingPosition_fst_mul_add_snd`, proves the block-cyclic row/column residue-cell facts `AIRA-T0011` through `AIRA-T0015` in Lean through `Circle.Applications.blockCyclicCell_fst_lt_blockSize`, `Circle.Applications.blockCyclicCell_snd_lt_blockSize`, `Circle.Applications.blockCyclicCell_add_row_blockSize`, `Circle.Applications.blockCyclicCell_add_column_blockSize`, and `Circle.Applications.blockCyclicCell_add_mul_blockSize`, and proves the two-period MultiCoil phase-pair/product-cycle facts `AIRA-T0016` through `AIRA-T0020` in Lean through `Circle.Applications.multiPhase2_fst_lt_periodA`, `Circle.Applications.multiPhase2_snd_lt_periodB`, `Circle.Applications.multiPhase2_zero`, `Circle.Applications.multiPhase2_add_periodProduct`, and `Circle.Applications.multiPhase2_add_mul_periodProduct`, with matching Python examples plus the exploratory adapter-block benchmark fixture `AIRA-B0001`, the exploratory MultiCoil/RoPE-style positional fixture `AIRA-B0002`, the exploratory RoPE-style relative phase fixture `AIRA-B0003`, the exploratory adapter parameter-budget fixture `AIRA-B0004`, the exploratory circulant-mixer validation fixture `AIRA-B0005`, the exploratory winding-aware position fixture `AIRA-B0006`, the exploratory block-cyclic mixer validation fixture `AIRA-B0007`, and the exploratory MultiCoil common-cycle closure fixture `AIRA-B0008`.
- These are finite indexing and bookkeeping primitives for AI prototypes plus starter phase-channel, learned-feature baseline, harmonic-feature baseline, backend-parity, cyclic-memory, coil-retrieval, content-gated retrieval, learned content-gate retrieval, looped-recurrence, token-level recurrence, learned token-level recurrence, training-free loop-wrapper, middle-block recurrence, learned middle-block recurrence, multi-resolution recurrence, learned multi-resolution recurrence, learned recurrence-schedule, adapter-block, circulant-mixer, block-cyclic-mixer, MultiCoil positional, relative phase, and winding-aware alias-control harnesses, not model-quality, parameter-efficiency, attention-replacement, RoPE, retrieval, recursive-reasoning, long-context, memory, hardware-efficiency, or runtime claims.
...
2. Coil Attention plus MultiCoil RoPE on synthetic long-context retrieval.
   Start from `AIM-B0002` as the coil-retrieval reachability scaffold, `AIRA-B0002` as the MultiCoil/RoPE-style positional scaffold, `AIRA-B0003` as the relative phase scaffold, and `AIRA-B0006` as the residue-plus-winding alias-control scaffold only. Compare against full attention, standard RoPE, learned positional encodings, sliding window, dilated attention, BigBird-like sparse attention, Hyena-like long convolution, recurrent memory, and S4/Mamba-like state-space baselines.

3. CoilRA adapters for small language-model fine-tuning.
   Start from `AIRA-B0001` as the adapter-block scaffold, `AIRA-B0004` as the parameter-budget scaffold, `AIRA-B0007` as the block-cyclic mixer dense-parity and alias/load scaffold, and `AIRA-B0008` as the MultiCoil common-cycle closure scaffold only. Compare against dense adapters, LoRA, block-circulant, circulant, standard RoPE, learned positional encodings, and learned dense-mixer baselines. Measure quality, parameter count, inference cost, memory, failure cases, and MLX runtime on this Mac.
