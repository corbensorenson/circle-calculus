...
- `PAPER_COMP_01_PROOF_CARRYING_CIRCULAR_COMPUTATION` proves the cyclic-address facts `COMPC-T0001` through `COMPC-T0004` in Lean through `Circle.Applications.cyclicAddress_lt_size`, `Circle.Applications.cyclicAddress_add_size`, `Circle.Applications.cyclicAddress_zero`, and `Circle.Applications.cyclicAddress_add_mul_size`, with matching Python examples.
- This is a proof-carrying rewrite primitive, not evidence that any backend is faster.
- `PAPER_COMP_02_COIL_RAY_AND_SAMPLING` proves the direction-bin schedule facts `COMPR-T0001` through `COMPR-T0004` in Lean through `Circle.Applications.directionBin_lt_binCount`, `Circle.Applications.directionBin_add_binCount`, `Circle.Applications.directionBin_zero`, and `Circle.Applications.directionBin_add_mul_binCount`, with matching Python examples.
...
- `PAPER_AI_01_CIRCLE_AI_ARCHITECTURES` proves the phase-channel facts `AIA-T0001` through `AIA-T0004` in Lean through `Circle.Applications.phaseChannel_lt_period`, `Circle.Applications.phaseChannel_add_period`, `Circle.Applications.phaseChannel_zero`, and `Circle.Applications.phaseChannel_add_mul_period`, with matching Python examples and the exploratory deterministic benchmark fixtures `AIA-B0001` and `AIA-B0002`.
