...
- `PAPER_P2_02_BOTT_CLIFFORD_PERIODICITY` proves the finite period-8 clock facts `P2B-T0001` through `P2B-T0003` in Lean through `Circle.Phase2.bottClockIndex_lt_eight`, `Circle.Phase2.bottClockIndex_add_eight`, and `Circle.Phase2.bottClockIndex_zero`, with matching Python examples.
...
- `PAPER_P2_03_BUNDLE_CALCULUS` proves the trivial product-bundle projection/fiber and base-preserving transition facts `P2BU-T0001` through `P2BU-T0007` in Lean through `Circle.Phase2.trivialBundleProjection_point`, `Circle.Phase2.trivialBundleFiber_point`, `Circle.Phase2.trivialBundleProjection_forgetsFiber`, `Circle.Phase2.trivialBundleFiber_forgetsBase`, `Circle.Phase2.bundleTransitionProjection_apply`, `Circle.Phase2.bundleTransitionFiber_apply`, and `Circle.Phase2.bundleTransitionApply_compose`, with matching Python examples.
...
- `PAPER_P2_04_BOUNDARY_COBORDISM_CALCULUS` proves the directed-interval boundary facts `P2BC-T0001` through `P2BC-T0004` in Lean through `Circle.Phase2.boundaryBoundaryInterval_zero`, `Circle.Phase2.intervalBoundary_reverse`, `Circle.Phase2.reverseInterval_involutive`, and `Circle.Phase2.intervalBoundary_constant_zero`, with matching Python examples.
- This is finite boundary bookkeeping, not a proof of general chain complexes, cobordism, TQFT, or physics boundary laws.
- `PAPER_P2_05_PROOF_CARRYING_GLYPHS` proves the proof-glyph certificate projection and finite metadata-validity facts `P2G-T0001` through `P2G-T0005` in Lean through `Circle.Phase2.proofGlyphTheoremId_mk`, `Circle.Phase2.proofGlyphLeanName_mk`, `Circle.Phase2.proofGlyphGlyphId_mk`, `Circle.Phase2.proofGlyphValidAgainst_resolves_metadata`, and `Circle.Phase2.proofGlyphValidAgainst_of_matching_metadata`, with matching Python examples.
...
- `PAPER_APP_01_COIL_DATA_ANALYSIS` proves the finite phase-coordinate facts `APPD-T0001` through `APPD-T0003` in Lean through `Circle.Applications.phaseCoordinate_lt_period`, `Circle.Applications.phaseCoordinate_add_period`, and `Circle.Applications.phaseCoordinate_zero`, with matching Python examples.
- This is a synthetic periodic-data primitive, not a real-data period detector or benchmark result.
- `PAPER_COMP_01_PROOF_CARRYING_CIRCULAR_COMPUTATION` proves the cyclic-address facts `COMPC-T0001` through `COMPC-T0003` in Lean through `Circle.Applications.cyclicAddress_lt_size`, `Circle.Applications.cyclicAddress_add_size`, and `Circle.Applications.cyclicAddress_zero`, with matching Python examples.
- This is a proof-carrying rewrite primitive, not evidence that any backend is faster.
- `PAPER_COMP_02_COIL_RAY_AND_SAMPLING` proves the direction-bin schedule facts `COMPR-T0001` through `COMPR-T0003` in Lean through `Circle.Applications.directionBin_lt_binCount`, `Circle.Applications.directionBin_add_binCount`, and `Circle.Applications.directionBin_zero`, with matching Python examples.
- This is finite queue/sampler indexing, not evidence of ray-coherence speedup, lower noise, or better frame time.
- `PAPER_COMP_03_COIL_LAYOUT_STENCIL_NTT` proves the stride-address facts `COMPL-T0001` through `COMPL-T0004` in Lean through `Circle.Applications.strideAddress_lt_size`, `Circle.Applications.strideAddress_add_size_steps`, `Circle.Applications.strideAddress_zero_step`, and `Circle.Applications.strideAddress_zero_stride`, with matching Python examples and a small CPU/optional-MLX CoilLayout benchmark harness.
