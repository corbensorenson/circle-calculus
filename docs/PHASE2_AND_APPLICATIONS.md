...
- `PAPER_APP_01_COIL_DATA_ANALYSIS` proves the finite phase-coordinate facts `APPD-T0001` through `APPD-T0003` in Lean through `Circle.Applications.phaseCoordinate_lt_period`, `Circle.Applications.phaseCoordinate_add_period`, and `Circle.Applications.phaseCoordinate_zero`, with matching Python examples. It also includes exploratory Python fixture `APPD-B0001`, comparing coil closure ranking with an autocorrelation baseline on a deterministic known-period signal.
