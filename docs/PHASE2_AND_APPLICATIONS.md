...
6. Theseus-Hive transfer.
   Use `docs/THESEUS_HIVE_AI_TRANSFER.md`, `P5-EDGE-011`, and `site/data/generated/theseus_hive_ai_contracts.json` as the bridge from Circle AI to the user's active AI system. The public Circle-side contract fixtures now cover recurrence schedules, strided fanout coverage, cyclic memory, phase tags, circulant/block-cyclic mixers, and seed-rule provenance. The companion Theseus-Hive workspace now has report-only consumers that load those fixtures and emit YELLOW private-planning, structural-smoke, deterministic-proxy, aggregate real-workload attachment, scored candidate-manifest, semantic score-context, full-frontier equal-budget semantic, and recurrence work-budget reports with zero external inference, no training mutation, no promotion evidence, six named smoke workload slots, six proxy benchmark ids, six aggregate local-artifact attachment ids, three scored fanout ids, and one scored recurrence id: `circle_candidate_fanout_scored_manifest_v1`, `circle_candidate_fanout_semantic_frontier_v1`, `circle_candidate_fanout_equal_budget_semantic_v1`, and `circle_recurrence_work_budget_semantic_v1`. The full-frontier semantic and recurrence runs both score 1040 private tasks across 26 categories but tie ordinary rank-one/fixed-depth baselines, so they are not Circle advantage claims; next design selection or recurrence workloads that differ from ordinary baselines when evidence supports them and promote the remaining aggregate attachments into scored private workloads inside Theseus-Hive against existing position-bucket, context-retention, dense/low-rank mixer, and object-storage baselines before claiming usefulness.
