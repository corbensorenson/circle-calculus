...
- Sparse update: `AIT-T0111` adds the deduplicated unique-lag necessary condition for complete coverage, so the sidecar can certify incompleteness from unique-lag shortfall even when raw generator budget is not the decisive field. `AIT-T0112` adds the candidate-range iff endpoint: when every generated lag candidate is positive and in context, complete coverage is equivalent to the unique lag-candidate count equaling `n - 1`. The next useful sparse step is a sharper gap-counting theorem or another iff with a concrete checkable hypothesis, not another one-sided report flag.
