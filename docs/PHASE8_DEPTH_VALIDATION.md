...
- Upgrade sparse-attention reachability lemmas into coverage iff statements. The current sparse lane has the abstract gap iff (`AIT-T0020`) and concrete no-local/no-stride-step uncovered-lag theorem (`AIT-T0021`); the next useful contract should add stronger coverage/no-gap conditions with candidate-budget accounting.
