...
- [x] `papers/applications/PAPER_AI_02_COIL_ATTENTION_AND_MEMORY.md`
  ...
  - Update: `AIT-T0106` and `AIT-T0107` add the general sparse-query count cap: for any nonempty context, the exact deduplicated query-candidate count is bounded by the context and by the reported `min(context, raw_budget)` clipped budget. `AIT-T0108` adds the finite-image bound that the query-indexed unique candidate count is never larger than the unique lag-candidate count. `AIT-T0109` adds the equality endpoint under predecessor injectivity on generated lag candidates.
