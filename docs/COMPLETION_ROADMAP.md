...
- [x] Add exact-regeneration self/symmetry/transitivity facts for generator comparisons, while keeping minimality and optimal-compression claims out of scope.
...
- [x] `P7-PHYS-001`: finite `U(1)`/`Z_n` lattice gauge Wilson-loop seed with Lean-proved `ZMod n` phase-list holonomy, link-path record holonomy, first source-target composability, checked finite path identity/associativity/holonomy, endpoint-gauge, closed-loop, and plaquette facts.
