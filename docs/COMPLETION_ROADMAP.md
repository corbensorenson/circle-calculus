...
- The dedicated Theseus-Hive AI transfer lane has converted the Circle AI spine into concrete recurrence, routing, memory, phase-feature, circulant-mixer, and seed-rule contract experiments, and the companion Theseus-Hive workspace now has report-only planning, structural-smoke, deterministic-proxy, aggregate real-workload attachment, scored candidate-manifest, semantic score-context, full-frontier equal-budget semantic, recurrence work-budget, context-packet memory, phase-feature ablation, and route/ranker mixer ablation consumers that load those contracts without importing private reports into this public repo or upgrading benchmark claims to proofs; the current scored phase ablation ties ordinary controls, while the mixer ablation ties dense heldout trace accuracy with lower counted parameters on a small fixture rather than proving deployed-router improvement, so the next private milestones are a genuinely different Circle selection, loop-depth, downstream memory-retrieval, route/ranker phase/mixer workload, and scored seed-rule workload rather than a public win claim.
