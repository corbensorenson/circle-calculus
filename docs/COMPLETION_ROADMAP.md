...
- [x] `papers/applications/PAPER_AI_04_ROPE_POSITION_CERTIFIER.md`
  ...
  - Status: draft; `AIRA-T0021` through `AIRA-T0207` plus `AIRA-T0210` through `AIRA-T0226` include exact integer-period collision/divisibility, LCM pass/fail, per-channel zero/positive count semantics, the single-channel period/context threshold, first-repeat-only count formulas, the quotient fitting-multiple bound, the fitting-range count reduction, the doubled triangular closed-form numerator, the exact divided closed form, total count semantics, bounded prefix/subfamily bridges, named diagnostic count certificates, finite nearest-integer margin certificates, exact weakest-gap report contracts, generic reduced-rational modular-inverse exact-gap reports, and standard channel-0 interval-backed margin seeds. Real-valued RoPE scans remain numerical diagnostics unless backed by explicit finite-margin theorem ids; model quality, context-length quality, speed, and memory claims remain out of scope.
