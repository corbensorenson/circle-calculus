...
- Phase VIII depth and external-validation work has turned the strongest proof-carrying contracts into deeper, externally useful artifacts: RoPE real-phase theorem work or a bounded quantitative precursor, sparse-attention coverage iffs or gap certificates, finite phase-bank collision counts, reproducible RoPE results on named configurations, one external checkpoint, a second end-to-end AI contract such as KV-cache/ring-buffer safety, proof-depth/vacuity guardrails, and ML-engineer-facing quickstarts.
- `make check` passes.
...
21. Build Phase VIII: depth, external validation, and contract hardening. Prioritize the RoPE real-phase margin theorem program, sparse-attention coverage iffs, finite phase-bank collision counting, reproducible RoPE results on named configurations, one outside review checkpoint, a second proof-carrying AI contract such as ring-buffer/KV-cache safety, a vacuity/proof-depth guard, root/CI visibility cleanup, and ML-engineer quickstarts.
...
- [x] `papers/applications/PAPER_AI_02_COIL_ATTENTION_AND_MEMORY.md`
  ...
  - Update: `AIT-T0139` through `AIT-T0144` make the public `4096` no-wrap and `8192` coprime planner rows row-specific: the covered/uncovered positive-lag counts are exactly `44`/`4051` and `96`/`8095`, and every query index has exactly `44` or `96` deduplicated predecessor candidates. `AIT-T0145` and `AIT-T0146` add lag/query dedup-loss zero iff no-collision endpoints, and the sidecar now includes a `C_16` alias-collision probe with nonzero loss.
