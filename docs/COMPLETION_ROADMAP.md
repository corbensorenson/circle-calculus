...
Verification rule:
After each meaningful stage, run the relevant checks. Use `make targeted-check-list` and `make targeted-check` for fast edit-loop validation, then use `make check` and any dimension-specific checks before release-grade commits or broad proof/status changes.
...
- The dedicated Theseus-Hive AI transfer lane has converted the Circle AI spine into concrete recurrence, routing, memory, phase-feature, circulant-mixer, and seed-rule contract experiments, and the companion Theseus-Hive workspace now has report-only planning, structural-smoke, deterministic-proxy, aggregate real-workload attachment, scored candidate-manifest, semantic score-context, full-frontier equal-budget semantic, recurrence work-budget, context-packet memory, phase-feature ablation, route/ranker mixer ablation, router-head mixer attachment, seed-rule exact-regeneration, seed-rule source-change stress, workflow/tool-card rebuild, and update-cycle effort consumers that load those contracts without importing private reports into this public repo or upgrading benchmark claims to proofs; the current scored phase ablation ties ordinary controls, the mixer ablation ties dense heldout trace accuracy with lower counted parameters on a small fixture, the router-head attachment ties current holdout-decision agreement with the same lower counted Circle parameter budget, the actual Octopus router-head trainer now has `circle_router_head_mixer_shadow_training_v1`, an opt-in Circle mixer aggregate shadow benchmark that ties the no-mixer trainer at 14/14 holdout exact-set accuracy with zero external inference on current local traces while recording 5-repetition stable timing across pure and residual Circle modes with median elapsed 10.704 ms baseline, 112.769 ms pure circulant, 96.332 ms pure block-cyclic, 100.91 ms residual circulant, and 97.362 ms residual block-cyclic plus median Python-traced peak bytes 117078, 335297, 339165, 447689, and 438394; its keyword-boundary contrastive diagnostic has 45 holdout examples where full-gain pure and residual Circle modes still trail the no-mixer baseline, while a quality-only synthetic gain sweep finds bounded residual gains 0.05, 0.1, 0.25, and 0.5 preserve synthetic baseline exactness, and a real-trace boundary split with 9 holdout examples finds full-gain Circle modes at 0.8889 exact accuracy while residual gains 0.05, 0.1, and 0.25 preserve baseline exactness, the seed-rule exact scorer regenerates safe provenance envelopes with zero verifier residuals and a shorter recipe than object-only storage, the seed-rule stress scorer records lower source-change edit cost than object-only storage while tying exactness and reuse, the workflow/tool-card rebuild scorer records lower automated rebuild work units than object-only storage while tying exactness and reuse, and the update-cycle effort instrumentor now finds review-step fields plus paired object-only/Circle maintenance-mode labels but still no human-edit-minute fields rather than proving deployed-router, model-quality, human-time, or universal-compression improvement; the private side now has a measured-effort recorder for future `human_edit_minutes` rows, so the next private milestones are a genuinely different Circle selection, loop-depth, downstream memory-retrieval, harder router-head phase/mixer training workload with profiling/failure-case evidence, and actual measured human-edit minutes on paired update-cycle maintenance rows rather than a public win claim.
...
- Phase VIII depth, contract-hardening, and change-aware-validation work has turned the strongest proof-carrying contracts into deeper, directly useful artifacts: RoPE real-phase theorem work or a bounded quantitative precursor, sparse-attention coverage iffs or gap certificates, finite phase-bank collision counts, reproducible RoPE results on named configurations, second and third end-to-end AI contracts such as KV-cache/ring-buffer safety and sparse-attention coverage, proof-depth/vacuity guardrails, ML-engineer-facing quickstarts, and a targeted validation runner that avoids full-suite checks when the changed dependency surface is local.
- `make check` passes.
...
- Theseus-Hive AI transfer plan, contracts, and companion consumer boundary: `docs/THESEUS_HIVE_AI_TRANSFER.md`, `circle_math/applications/theseus_hive_contracts.py`, `scripts/export_theseus_hive_ai_contracts.py`, `site/data/generated/theseus_hive_ai_contracts.json`, and `P5-EDGE-011` in `manifests/phase5_edge_targets.yaml`
...
- Phase VIII depth/validation registry: `manifests/phase8_depth_validation.yaml`
- Phase VIII depth/validation validator: `scripts/check_phase8_targets.py`
- Phase VIII context: `docs/PHASE8_DEPTH_VALIDATION.md`
- AI contract suite entry points: `docs/AI_CONTRACT_SUITE.md` and `site/chapters/applications/ai_contract_suite.qmd`
- Change-aware validation runner and policy: `scripts/targeted_check.py` and `docs/CHANGE_AWARE_VALIDATION.md`
...
21. Build Phase VIII: depth, contract hardening, and change-aware validation. Prioritize the RoPE real-phase margin theorem program, sparse-attention coverage iffs, finite phase-bank collision counting, reproducible RoPE results on named configurations, proof-carrying AI contracts such as ring-buffer/KV-cache safety and sparse-attention coverage, looped/recursive transformer schedule contracts, a vacuity/proof-depth guard, targeted validation for fast local iteration, root/CI visibility cleanup, and ML-engineer quickstarts. External review/checkpoints are optional and should not be pursued unless explicitly requested.
22. If the roadmap is exhausted or blocked, search authoritative sources for additional theorem targets and convert the best candidates into manifests, dictionary entries, papers, Lean/Python sidecars, and Living Book links.
...
- [x] `papers/S1/PAPER_S1_01_FINITE_CIRCLES.md`
  ...
  - Status: polished dimension guide draft links the proved theorem spine into the dimension layout while keeping the root paper as the detailed source. The full same-orbit iff gcd-congruence representative theorem is Lean-proved as `CC-T0061`; `CC-T0062` through `CC-T0072` add separate mathlib-backed Erdős-Ginzburg-Ziv, Cauchy-Davenport, Katona/Erdős-Ko-Rado, Roth, Hales-Jewett/van der Waerden, cycle/circulant graph, and empty unit-distance boundary bridges; `P5-EDGE-002` adds an exploratory finite-coil theorem-signature index for proof navigation.
...
- Status: polished draft; `AIM-T0001` through `AIM-T0127` are Lean-proved cyclic-memory-slot, KV-cache ring-buffer, finite loop-schedule, active-token-step/list exactness, active/inactive recurrence work-count accounting, numeric work-count monotonicity, public active/inactive fixture counts, middle-block-route, combined route/budget one-cycle and multi-cycle closure plus sample-zero initialization, looped recurrent-state, positive raw-budget closure, multi-pass recurrence closure, training-free loop-budget, unavailable-budget clamp, overthinking-boundary, loop-exit certificate, certificate-availability, certificate-budget-selection, no-exit/no-active iff facts, live-window/full-coverage iff facts, generated-live-window request contracts, duplicate-free ordered live-window subrequest pass contracts, compact-boundary retained/slot-distinct consequence contracts, exact stale-member success/failure iff contracts, counted stale-request success/failure contracts, pinned seen-prefix plus rolling-window sink-policy membership contracts, retained rolling-suffix freshness, general duplicate-free sink-window request contracts, sink-window request-size budget facts, and sink-window exact-count condition facts with Python examples; `AIT-T0010` through `AIT-T0170` are Lean-proved local/coil/hybrid sparse-attention reachability, gap, coverage-threshold, family-recursion, empty-family, lag-preservation, complete-coverage/no-uncovered-lag equivalence, complete-coverage budget-preservation, raw candidate-budget upper-bound, raw-budget necessary condition for complete coverage, context-clipped deduplicated-budget, exact theorem-side lag-candidate membership, query-indexed candidate-list/count, query-count context/clipped-budget/image-cardinality bounds and injective-map equality endpoint, lag-side and query-side unique-count shortfall iff gap endpoints, no-collision equality and iff endpoints, lag/query dedup-loss zero/positive iff no-collision/collision endpoints, unique-plus-loss-equals-raw accounting endpoints, public fixture pair-collision count endpoints, reusable pair-count zero/positive iff no-collision/collision endpoints, pair-count-bounds-dedup-loss severity inequalities, structural no-collision sufficient-condition, singleton no-wrap numeric sufficient-condition, singleton/family no-zero period-threshold iff, period-threshold shortfall/gap endpoints, zero-residue period-violation witness iff, no-zero failure iff endpoints, first-alias period witness facts, zero-residue alias-count and total-count formulas, public 4096 no-wrap and 8192 coprime covered/uncovered and query-side exact-count facts, compositional multi-stride residue no-collision, numeric head/tail separation, ordered separated-family residue no-collision, separated-family local/coil disjointness, separated-family full lag-candidate no-collision, separated-family query no-collision, numeric predecessor-injectivity, exact raw-budget endpoint facts, finite positive-lag range coverage iff, finite uncovered/covered-lag list iff/empty/count/partition/count-witness/shortfall-witness/first-gap facts, concrete default-plan gap/incomplete-coverage/count/first-gap facts, public sparse-fixture uncovered-interval summary facts, first-gap/first-interval local repair boundary facts, and default-row complete-repair minimality facts; `AIM-B0001` is an exploratory cyclic-memory fixture with constant/scalar-threshold baselines and a nonperiodic control; `AIM-B0002` is an exploratory coil-retrieval reachability fixture with coil-path, local-window, wrong-stride, full-attention oracle, and near-lag controls; `AIM-B0003` is an exploratory looped-recurrence schedule fixture with single-pass, fixed-loop, adaptive-exit, recurrent-memory, sparse phase-router, over-looped, and dense-threshold controls; `AIM-B0011` is an exploratory loop-exit certificate fixture with score trace, exit availability, budget status, no-exit/no-active status, and overthinking guardrail status; `AIM-B0004` is an exploratory content-gated retrieval fixture with static coil, static local, wrong-gate, union-candidate, full-attention, and candidate-budget controls; `AIM-B0016` is an exploratory hybrid local+coil sparse-attention fixture with local-only, coil-only, wrong-stride hybrid, full-attention, structured mixed-lag, nonstructured-control, and candidate-budget diagnostics; `AIM-B0017` is an exploratory stride-family sparse-attention fixture with local-only, single-stride, wrong-family, full-attention, nonstructured-control, coverage-certificate, gap-witness, no-collision, interval-summary, first-interval next-gap fields, complete-repair minimality fields, raw-budget shortfall, unique-query shortfall, singleton/family period-threshold fields, zero-residue witness/failure fields, first-zero witness fields, zero-residue per-stride and total-count fields, lag/query dedup-loss fields, exact accounting fields, collision-pair count fields, pair-count boundary/severity fields, alias-collision probe, and budget diagnostics; `AIM-B0005` is an exploratory token-level recurrence routing fixture with per-token budgets, active-token counts, selected middle block, resolution labels, fixed/wrong/over-loop controls, and nonperiodic scalar-threshold control; `AIM-B0012` is an exploratory learned token-level recurrence fixture with a phase-to-budget lookup, fixed-budget baseline, wrong-period control, shifted-budget control, over-loop control, and nonperiodic scalar-threshold control; `AIM-B0006` is an exploratory training-free loop-wrapper fixture with circular phase budgets, fixed-loop, wrong-period, over-loop, scalar-threshold baselines, and CPU/optional-MLX scoring; `AIM-B0007` is an exploratory middle-block recurrence fixture with selected-block, full-block, fixed-budget, wrong-block, over-loop controls, and block-pass accounting; `AIM-B0013` is an exploratory learned middle-block fixture with phase-to-block and phase-to-budget lookup tables, selected-band/full-block baselines, fixed-budget, wrong-period, wrong-block, over-loop controls, and block-pass accounting; `AIM-B0008` is an exploratory multi-resolution recurrence fixture with coarse/fine phase routing, single-resolution, fixed-budget, wrong-resolution, over-loop controls, and active-sample accounting; `AIM-B0014` is an exploratory learned multi-resolution fixture with phase-to-budget and phase-to-resolution lookup tables, single-resolution/fixed-budget baselines, wrong-period controls, over-loop controls, and active-sample accounting; `AIM-B0009` is an exploratory learned recurrence-schedule fixture with a phase-to-budget lookup, fixed-budget baseline, wrong-period control, and over-loop control; `AIM-B0015` is an exploratory tiny looped recurrent-state prototype with direct phase, one-step, scalar-threshold, wrong-period, and nonperiodic controls; `AIM-B0010` is an exploratory learned content-gate retrieval fixture with a phase-to-route lookup, wrong-period and flipped-gate controls, route accuracy, and candidate-budget diagnostics; retrieval quality, alias control, attention replacement, recursive reasoning, and looped-model quality claims remain benchmark work.
  - Update: `AIT-T0085` adds the exact default sparse-plan uncovered-lag count theorem, proving that the `C_120`, local-window `4`, path-length `3`, strides `[7,13]` fixture has `109` uncovered positive in-context lags.
  - Update: `AIT-T0106` and `AIT-T0107` add the general sparse-query count cap: for any nonempty context, the exact deduplicated query-candidate count is bounded by the context and by the reported `min(context, raw_budget)` clipped budget. `AIT-T0108` adds the finite-image bound that the query-indexed unique candidate count is never larger than the unique lag-candidate count. `AIT-T0109` adds the equality endpoint under predecessor injectivity on generated lag candidates. `AIT-T0110` adds the raw-budget necessary condition for complete coverage: raw budget below `n - 1` certifies incompleteness.
  - Update: `AIT-T0111` adds the deduplicated unique-lag necessary condition for complete coverage: if duplicate removal leaves fewer unique lag candidates than positive in-context lags, the sparse plan cannot be complete.
  - Update: `AIT-T0112` adds a candidate-range iff endpoint: if every generated lag candidate is positive and in context, complete coverage is equivalent to the deduplicated unique lag-candidate count equaling `n - 1`.
  - Update: `AIT-T0113` and `AIT-T0114` add the candidate-range count formulas: covered count equals unique lag-candidate count, and uncovered count equals `n - 1` minus that unique count.
  - Update: `AIT-T0115` and `AIT-T0116` add the no-wrap separated structural discharge for that candidate-range hypothesis: `window < context` plus an ordered no-wrap separated stride family proves every generated lag candidate is positive and in context, then gives the same unique-count iff under those checkable plan conditions.
  - Update: `AIT-T0117` through `AIT-T0119` add the broader no-zero-residue structural discharge: the plan may wrap or overlap, but `window < context` plus every admitted stride-step residue being nonzero proves candidate range and gives the same unique-count coverage iff.
  - Update: `AIT-T0120` through `AIT-T0125` add the failure-side shortfall iff endpoints: under candidate-range and the needed query injectivity, plus no-wrap/no-zero structural wrappers, lag-side and query-indexed unique-count shortfall is equivalent to a concrete uncovered positive lag. `AIT-T0126` through `AIT-T0128` make the no-zero structural check exact by period thresholds: singleton plans use one period, finite families require every admitted stride period to exceed `pathLength`. `AIT-T0129` and `AIT-T0130` package those period thresholds as lag-side and query-side shortfall/gap equivalence checks. `AIT-T0131` adds the matching zero-residue witness iff for period-threshold violations, `AIT-T0132`/`AIT-T0133` make the no-zero failure booleans exact, and `AIT-T0134`/`AIT-T0135` make the reported period witness first-alias exact.
  - Update: `AIT-T0136` through `AIT-T0138` add the count-level companion for zero-residue sparse aliases: for one stride, the admitted positive steps returning to zero residue are counted exactly by `pathLength / period`; for a finite stride family, the total count is the sum of those per-stride quotients; and in nonzero contexts total count zero is equivalent to the no-zero structural condition. The certifier now emits per-stride and family-total count fields for those formulas.
  - Update: `AIT-T0139` through `AIT-T0144` make the public `4096` no-wrap and `8192` coprime planner rows row-specific: the covered/uncovered positive-lag counts are exactly `44`/`4051` and `96`/`8095`, and every query index has exactly `44` or `96` deduplicated predecessor candidates. `AIT-T0145` through `AIT-T0150` add lag/query dedup-loss zero/positive iff no-collision/collision endpoints plus unique-plus-loss-equals-raw accounting, and the sidecar now includes a `C_16` alias-collision probe with positive loss. `AIT-T0151` through `AIT-T0154` add public fixture pair-collision counts for the default zero-collision plan and the compact six-pair alias probe; `AIT-T0155` through `AIT-T0158` add reusable pair-count zero/positive iff no-collision/collision endpoints for lag and query candidates; `AIT-T0159` and `AIT-T0160` add the lag/query pair-count-bounds-dedup-loss severity inequalities consumed by the public sparse certifier; `AIT-T0161` through `AIT-T0167` make the first-gap and first-interval local repair boundaries theorem-backed, `AIT-T0172` proves the reusable final-lag condition under which dense-local fallback is minimal, and `AIT-T0168` through `AIT-T0170` prove that the default row's complete repair window `119` is exact and minimal for the declared path/stride family.
- [x] `papers/applications/PAPER_AI_03_COILRA_AND_MULTICOIL_ROPE.md`
  ...
  - Status: polished draft; `AIRA-T0001` through `AIRA-T0005` are Lean-proved adapter-block facts with Python examples, `AIRA-T0006` through `AIRA-T0010` are Lean-proved residue/winding position facts with Python examples, `AIRA-T0011` through `AIRA-T0015` are Lean-proved block-cyclic row/column residue-cell facts with Python examples, and `AIRA-T0016` through `AIRA-T0020` are Lean-proved two-period MultiCoil phase-pair/product-cycle closure facts with Python examples; `AIRA-B0001` is an exploratory adapter-block fixture with constant/scalar-threshold baselines and a nonperiodic control; `AIRA-B0002` is an exploratory MultiCoil/RoPE-style positional fixture with single-period, constant, scalar-threshold, and nonperiodic controls; `AIRA-B0003` is an exploratory RoPE-style relative phase fixture with wrong-period, query-position, scalar-threshold, and nonperiodic controls; `AIRA-B0004` is an exploratory adapter parameter-budget fixture with dense adapter, LoRA-style low-rank, block-cyclic shared-table, and alias/load diagnostics; `AIRA-B0005` is an exploratory circulant-mixer validation fixture with dense circulant-matrix parity, wrong-shift control, and parameter-count diagnostics; `AIRA-B0006` is an exploratory winding-aware position fixture with residue-only, wrong-period, learned absolute-position, scalar-threshold, alias-diagnostic, and nonperiodic controls; `AIRA-B0007` is an exploratory block-cyclic mixer validation fixture with dense-matrix parity, wrong-row-shift control, parameter-count diagnostics, and cell-alias/load diagnostics; `AIRA-B0008` is an exploratory MultiCoil common-cycle closure fixture with product-cycle, lcm, and wrong-shift controls; CoilLinear, CoilRA, MultiCoil RoPE, model quality, parameter efficiency, memory, training stability, context-length quality, and runtime claims remain benchmark work.
- [x] `papers/applications/PAPER_AI_04_ROPE_POSITION_CERTIFIER.md`
  - Program: proof-carrying RoPE position distinguishability, integer-period phase-bank collision counts, finite turn-ratio margin certificates, standard channel-0 interval seeds, and public preset reports.
  - Status: draft; `AIRA-T0021` through `AIRA-T0207` plus `AIRA-T0210` through `AIRA-T0231` include exact integer-period collision/divisibility, LCM pass/fail, per-channel zero/positive count semantics, the single-channel period/context threshold, first-repeat-only count formulas, the quotient fitting-multiple bound, the fitting-range count reduction, the doubled triangular closed-form numerator, the exact divided closed form, total count semantics, bounded prefix/subfamily bridges, named diagnostic count certificates, finite nearest-integer margin certificates, exact weakest-gap report contracts, generic reduced-rational modular-inverse exact-gap reports, the full-denominator reduced-rational exact-gap existence, exact-threshold, request-obstruction, and generic exact weakest-gap request-threshold theorems, and standard channel-0 interval-backed margin seeds. Real-valued RoPE scans remain numerical diagnostics unless backed by explicit finite-margin theorem ids; model quality, context-length quality, speed, and memory claims remain out of scope.
...
- [x] `AIA-B0005`: exploratory harmonic/Fourier-feature fixture with correct-frequency, wrong-frequency, scalar, learned-position, and nonperiodic controls.
...
- [x] `AIM-T0011`: training-free loop budget is capped by the required loop depth.
- [x] `AIM-T0012`: loop overthinking boundary is at least the required loop depth.
- [x] `AIM-T0013`: full-period loop budget makes exit available.
- [x] `AIM-T0014`: loop-exit availability closes after one full loop-period shift.
- [x] `AIM-T0015`: loop-exit certificate records the exact required step.
- [x] `AIM-T0016`: loop-exit certificate records the budget bound.
- [x] `AIM-T0017`: loop-exit certificate records the overthinking guardrail bound.
- [x] `AIM-T0018`: token recurrence budget is bounded by the positive loop period.
- [x] `AIM-T0019`: training-free loop budget closes after one full loop-period shift.
- [x] `AIM-T0020`: available loop-exit budget makes the training-free wrapper choose the exact required depth.
- [x] `AIM-T0021`: loop overthinking boundary closes after one full loop-period shift.
- [x] `AIM-T0022`: token recurrence budgets are positive.
- [x] `AIM-T0023`: available training-free loop budgets are positive.
- [x] `AIM-T0024`: loop-exit certificates carry positive exit steps.
- [x] `AIM-T0025`: unavailable training-free loop budgets clamp to `maxLoops`.
- [x] `AIM-T0026`: looped-recurrence required depth closes after any whole number of loop-period passes.
- [x] `AIM-T0027`: token recurrence budgets close after any whole number of loop-period passes.
- [x] `AIM-T0028`: training-free loop budgets close after any whole number of loop-period passes.
- [x] `AIM-T0029`: loop overthinking boundaries close after any whole number of loop-period passes.
- [x] `AIM-T0030`: loop-exit availability closes after any whole number of loop-period passes.
- [x] `AIM-T0031`: loop-exit certificates imply exit availability.
- [x] `AIM-T0032`: loop-exit certificates make the training-free wrapper choose the certified exit step.
- [x] `AIM-T0033`: loop-exit certificate exit steps close after any whole number of loop-period passes.
- [x] `AIM-T0034`: loop-exit certificate guardrail boundaries close after any whole number of loop-period passes.
- [x] `AIM-T0035`: every token is active at the first one-indexed loop step.
- [x] `AIM-T0036`: token active-step membership closes after any whole number of loop-period token shifts.
- [x] `AIM-T0037`: active token steps are bounded by the positive loop period.
- [x] `AIM-T0038`: token steps beyond the loop period are inactive.
- [x] `AIM-T0039`: selected middle-block routes stay at or after the selected range start.
- [x] `AIM-T0040`: selected middle-block routes stay before the selected range stop.
...
- [x] `AIM-T0056`: combined middle-block/budget routes close after any whole number of product common cycles.
- [x] `AIM-T0057`: token recurrence budgets at token/sample zero start with one loop step.
- [x] `AIM-T0058`: combined middle-block/budget route sample zero anchors at `(start, 1)`.
- [x] `AIM-T0049`: looped recurrent hidden-state indices are bounded by a positive recurrent period.
- [x] `AIM-T0050`: a one-step looped recurrent hidden state starts at zero.
- [x] `AIM-T0051`: reading a looped recurrent state at the required loop depth recovers the sample phase.
- [x] `AIM-T0052`: reading a looped recurrent state at the certified token recurrence budget recovers the token phase.
- [x] `AIM-T0053`: certified looped recurrent state reads are invariant under whole loop-period token shifts.
- [x] `AIM-T0054`: positive raw looped recurrent-state budgets close after one whole recurrent period.
- [x] `AIM-T0055`: positive raw looped recurrent-state budgets close after any whole number of recurrent periods.
...
- [x] `AIRA-T0006`: position residue is bounded by a positive period.
- [x] `AIRA-T0007`: position residue closes after one full period.
- [x] `AIRA-T0008`: position residue closes after any whole number of full periods.
- [x] `AIRA-T0009`: winding times period plus residue reconstructs a natural-number position.
- [x] `AIRA-T0010`: the paired winding-position representation reconstructs the original natural-number position.
- [x] `AIRA-T0011`: block-cyclic row residue cells are bounded by the positive block size.
- [x] `AIRA-T0012`: block-cyclic column residue cells are bounded by the positive block size.
- [x] `AIRA-T0013`: block-cyclic cells close after one whole row block-size pass.
- [x] `AIRA-T0014`: block-cyclic cells close after one whole column block-size pass.
- [x] `AIRA-T0015`: block-cyclic cells close after any whole number of row and column block-size passes.
- [x] `AIRA-T0016`: first coordinate of a two-period MultiCoil phase pair is bounded by the first positive period.
- [x] `AIRA-T0017`: second coordinate of a two-period MultiCoil phase pair is bounded by the second positive period.
- [x] `AIRA-T0018`: the two-period MultiCoil phase pair at position zero is the zero pair.
- [x] `AIRA-T0019`: two-period MultiCoil phase pairs close after one product-cycle pass.
- [x] `AIRA-T0020`: two-period MultiCoil phase pairs close after any whole number of product-cycle passes.
...
- [x] `AIRA-B0004`: exploratory adapter parameter-budget fixture with dense/LoRA/block-cyclic counts and alias diagnostics.
- [x] `AIRA-B0005`: exploratory circulant-mixer validation fixture with dense parity and a wrong-shift control.
- [x] `AIRA-B0006`: exploratory winding-aware position fixture with residue/winding alias controls.
- [x] `AIRA-B0007`: exploratory block-cyclic mixer validation fixture with dense parity, wrong-row-shift, and alias/load diagnostics.
- [x] `AIRA-B0008`: exploratory MultiCoil common-cycle closure fixture with product-cycle, lcm, and wrong-shift controls.
...
- [x] `CC-T0064` through `CC-T0072`: Katona/Erdős-Ko-Rado, Roth, Hales-Jewett/van der Waerden, cycle/circulant graph, and empty unit-distance boundary bridges as mathlib-backed external theorem wrappers
...
- [ ] Additive combinatorics and Erdős-facing finite-circle bridges: can zero-sum, sumset, and representation-count theorems over `C n` become proof-linked Circle examples without claiming new proofs or open-problem progress?
...
- [ ] AI/ML inductive bias: can phase channels, cyclic memory, coil/sparse attention, MultiCoil RoPE, adapter scheduling, looped/recursive transformer recurrence, recurrent/state-space/convolutional sequence models, harmonic/circulant features, geometry-aware representations, or proof-carrying model components create measurable gains on tasks with real periodic, rotational, memory-like, recurrent, or fibered structure?
...
- [x] `P5-EDGE-009`: full-spectrum Circle AI program target separates proof targets, benchmarks, baselines, controls, model prototypes, and speculative hypotheses; `AIA-B0003` adds CPU/optional-MLX backend parity scaffolding, `AIA-B0004` adds learned-feature baseline controls, `AIA-B0005` adds harmonic/Fourier-feature baseline controls, `AIM-B0002` extends the starter fixtures to coil-retrieval reachability, `AIM-B0003` adds looped-recurrence schedule controls, `AIM-B0011` adds loop-exit certificate controls, `AIM-B0004` adds content-gated retrieval routing, `AIM-B0005` adds token-level recurrence routing, `AIM-B0012` adds learned token-level recurrence routing, `AIM-B0006` adds training-free loop-wrapper controls, `AIM-B0007` adds middle-block recurrence controls, `AIM-B0013` adds learned middle-block recurrence controls, `AIM-B0008` adds multi-resolution recurrence controls, `AIM-B0014` adds learned multi-resolution recurrence controls, `AIM-B0009` adds learned recurrence-schedule controls, `AIM-B0015` adds a tiny looped recurrent-state prototype, `AIM-B0010` adds learned content-gate retrieval controls, `AIRA-B0001` extends them to adapter-block structure, `AIRA-B0002` extends them to MultiCoil/RoPE-style positional structure, `AIRA-B0003` adds RoPE-style relative phase controls, `AIRA-B0004` adds adapter parameter-budget controls, `AIRA-B0005` adds circulant-mixer validation, `AIRA-B0006` adds winding-aware alias-control position fixtures, `AIRA-B0007` adds block-cyclic mixer dense-parity and alias/load controls, and `AIRA-B0008` adds MultiCoil common-cycle closure controls.
...
- [x] Harmonic/Fourier feature scaffolding for correct-frequency, wrong-frequency, scalar, learned-position, and nonperiodic controls.
...
- [x] Circulant-mixer structural validation and parameter-count fixture.
...
- [x] Deterministic token-level recurrence routing fixture with active-token counts, selected middle block, resolution labels, and wrong/over-loop controls.
- [x] Deterministic learned token-level recurrence fixture with phase-to-budget lookup, fixed-budget baseline, wrong-period control, shifted-budget control, over-loop control, and nonperiodic scalar-threshold control.
- [x] Deterministic learned middle-block recurrence fixture with phase-to-block lookup, phase-to-budget lookup, selected-band/full-block baselines, fixed-budget, wrong-period, wrong-block, over-loop controls, and block-pass accounting.
- [x] Deterministic learned multi-resolution recurrence fixture with phase-to-budget lookup, phase-to-resolution lookup, single-resolution/fixed-budget baselines, wrong-period controls, over-loop controls, and active-sample accounting.
- [ ] Real learned recurrence prototypes against ordinary looped and nonlooped transformer baselines.
...
- [x] `AIA-B0005`: harmonic/Fourier-feature fixture comparing correct sine/cosine phase lookup with wrong-frequency, scalar, and learned-position baselines.
...
- [x] `AIM-B0016`: hybrid local+coil sparse-attention fixture with local-only, coil-only, wrong-stride hybrid, full-attention, structured mixed-lag, nonstructured-control, and candidate-budget diagnostics.
- [x] `AIM-B0017`: stride-family sparse-attention fixture with local-only, single-stride, wrong-family, full-attention, nonstructured-control, coverage-certificate, gap-witness, no-collision, and budget diagnostics.
...
- [x] `AIM-B0012`: learned token-level recurrence fixture with phase-to-budget lookup, fixed-budget baseline, wrong-period control, shifted-budget control, over-loop control, and nonperiodic scalar-threshold control.
...
- [x] `AIM-B0013`: learned middle-block recurrence fixture with phase-to-block lookup, phase-to-budget lookup, selected-band/full-block baselines, fixed-budget, wrong-period, wrong-block, over-loop controls, and block-pass accounting.
...
- [x] `AIM-B0014`: learned multi-resolution recurrence fixture with phase-to-budget lookup, phase-to-resolution lookup, single-resolution/fixed-budget baselines, wrong-period controls, over-loop controls, and active-sample accounting.
...
- [x] `AIM-B0015`: tiny looped recurrent-state prototype with direct phase, one-step, scalar-threshold, wrong-period, and nonperiodic controls.
...
- [x] `AIM-B0011`: loop-exit certificate fixture with score trace, exit availability, budget status, no-exit/no-active status, overthinking guardrail status, and a fixed-budget no-exit control.
...
- [x] `AIRA-B0004`: adapter parameter-budget fixture with dense adapter, LoRA-style low-rank, block-cyclic shared-table, and alias/load diagnostics.
- [x] `AIRA-B0005`: circulant-mixer validation fixture with dense circulant-matrix parity, wrong-shift control, and parameter-count diagnostics.
- [x] `AIRA-B0006`: winding-aware position fixture with residue-only, wrong-period, learned absolute-position, scalar-threshold, alias-diagnostic, and nonperiodic controls.
- [x] `AIRA-B0007`: block-cyclic mixer validation fixture with dense-matrix parity, wrong-row-shift control, parameter-count diagnostics, and cell-alias/load diagnostics.
- [x] `AIRA-B0008`: MultiCoil common-cycle closure fixture with product-cycle, lcm, and wrong-shift controls.
...
- [x] Promote the tractable finite theorem ids for path holonomy concatenation, reversal, link-path record holonomy, singleton/two-link/singleton-concat/three-link/four-link, empty-identity, concat-identity, concat-associativity, reversal-algebra facts, boundary-append composability, nonempty source/target projection, checked finite path endpoints, checked finite path identity/associativity/holonomy, checked-to-link-path projection bridges, checked closed-loop gauge-invariance facts, two-path and complete cyclic three-path closed-cycle holonomy/basepoint invariance facts, closed-loop gauge-sample independence, gauge-transformed endpoint behavior, closed-loop gauge invariance, and plaquette invariance to Lean-proved `ZMod n` declarations.
...
- [x] Add a Living Book physics lesson path: finite gauge loops -> holonomy -> Hopf hidden phase -> spin sign ambiguity -> periodic winding dynamics -> winding-defect toys.
...
- [x] Strengthen finite-circle seed generation with exact node membership, duplicate-free output, zero-size empty output, and positive-size nonempty output facts.
...
- [x] Add bounded finite-list search gates for exact-candidate list soundness, best-exact existence, singleton membership, and finite-search count bounds without promoting them to global optimality claims.
