...
- [x] `papers/applications/PAPER_AI_02_COIL_ATTENTION_AND_MEMORY.md`
  ...
  - Update: `AIT-T0136` adds the count-level companion for zero-residue sparse aliases: for one stride, the admitted positive steps returning to zero residue are counted exactly by `pathLength / period`, and the certifier now emits per-stride count fields for that formula.
