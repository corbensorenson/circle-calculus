...
- [x] `papers/applications/PAPER_AI_02_COIL_ATTENTION_AND_MEMORY.md`
  ...
  - Update: `AIT-T0120` through `AIT-T0125` add the failure-side shortfall iff endpoints: under candidate-range and the needed query injectivity, plus no-wrap/no-zero structural wrappers, lag-side and query-indexed unique-count shortfall is equivalent to a concrete uncovered positive lag. `AIT-T0126` and `AIT-T0127` make the singleton no-zero structural check exact by the period threshold `pathLength < period`.
