...
- The dedicated Theseus-Hive AI transfer lane has converted the Circle AI spine into concrete recurrence, routing, memory, phase-feature, circulant-mixer, and seed-rule contract experiments, and the companion Theseus-Hive workspace now has report-only planning, structural-smoke, deterministic-proxy, aggregate real-workload attachment, and scored candidate-manifest consumers that load those contracts without importing private reports into this public repo or upgrading benchmark claims to proofs.
