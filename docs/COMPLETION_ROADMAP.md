...
- [x] `papers/applications/PAPER_AI_02_COIL_ATTENTION_AND_MEMORY.md`
  ...
  - Update: `AIT-T0106` and `AIT-T0107` add the general sparse-query count cap: for any nonempty context, the exact deduplicated query-candidate count is bounded by the context and by the reported `min(context, raw_budget)` clipped budget.
