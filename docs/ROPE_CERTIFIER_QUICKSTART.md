...
```text
proof_layers=exact_integer_period_phase_bank:PASS,rational_discretized_finite_margin:AVAILABLE_NAMED_PRESET,interval_backed_standard_rope:AVAILABLE_SEED_CONTEXT_131072,numerical_real_phase_scan:PASS
...
`proof_layers=...` separates the evidence types. The exact integer-period layer is theorem-backed for the declared discretized phase-bank model. The rational/discretized finite-margin layer points to the named `1/4099` preset. The interval-backed standard-RoPE layer points to the bounded channel-0 context-131072 seed. The numerical scan remains diagnostic even when it passes.
...
```text
standard_rope_channel0_interval_context_131072
True
1/328459
AIRA-T0063,AIRA-T0064,AIRA-T0065,...,AIRA-T0161
```

Lean proves that channel 0 with standard turn ratio `1 / (2π)` has finite-context nearest-integer margin `1/328459` for gaps `1` through `131071`. The D17 128k seed uses the 20-decimal enclosure `10^20*gap/628318530717958647694 <= gap/(2π) <= 10^20*gap/628318530717958647692`, split across computed integer cells `0` through `20860`. Lean also proves the sharper 64k bracket at margin `1/104219`, while every advertised margin at or above `1/104218` is impossible there because of gap `710`. `AIRA-T0154` and `AIRA-T0155` add the 128k adjacent obstruction: gap `103993` is already within `1/328458` of integer turn `16551`, so `1/328458` and larger margins are impossible once that gap is in scope. `AIRA-T0156` through `AIRA-T0161` are the generated D17 128k interval certificate, finite-margin theorem, one-channel no-near-turn theorem, conditional bank bridges, and 128k bracket. This is real standard-RoPE theorem content, but still channel-0 based; it is not a proof for every channel in the whole multi-channel bank.

To ask the strongest concrete D17 bank-bridge request directly:

```python
from fractions import Fraction
from circle_math.applications import certify_standard_channel0_d17_bank_request

request = certify_standard_channel0_d17_bank_request(
    requested_context=131072,
    requested_margin=Fraction(1, 328459),
)
request.pass_certificate  # True
request.theorem_ids       # AIRA-T0159,AIRA-T0160
request.tolerance_rule    # tolerance < fullTurn * requestedMargin
```

This request certificate checks only that the requested context and margin fit inside the D17 seed and that the bank has the stated standard-channel-0 shape. It does not prove every channel has an independent margin.

To read the D17 margin bracket directly:

```python
from circle_math.applications import certify_standard_channel0_d17_margin_bracket

bracket = certify_standard_channel0_d17_margin_bracket()
bracket.context_length             # 131072
bracket.proved_margin              # 1/328459
bracket.impossible_margin_floor    # 1/328458
bracket.theorem_ids                # AIRA-T0156,AIRA-T0157,AIRA-T0155,AIRA-T0161
```
...
The d4, d6, conservative d20 `1/131072`, tighter d20 `1/105000`, sharp 4k d20 `1/104219`, weaker 8k d20 `1/104220`, sharp 8k d20 `1/104219`, sharp 16k d20 `1/104219`, sharp 32k d20 `1/104219`, sharp 64k d20 `1/104219`, and D17 128k d20 `1/328459` plans have been converted into Lean proof. The stronger 128k request at margin `1/104219` still reaches first uncovered gap `103993`, so its audit remains a failed frontier comparison.
