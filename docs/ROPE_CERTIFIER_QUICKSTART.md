...
```text
proof_layers=exact_integer_period_phase_bank:PASS,rational_discretized_finite_margin:AVAILABLE_NAMED_PRESET,interval_backed_standard_rope:AVAILABLE_SEED_CONTEXT_8,numerical_real_phase_scan:PASS
...
```text
standard_rope_channel0_interval_context_8
True
1/32
AIRA-T0063,AIRA-T0064,AIRA-T0068,AIRA-T0069,AIRA-T0070,AIRA-T0071,AIRA-T0072,AIRA-T0073
```

Lean proves that channel 0 with standard turn ratio `1 / (2π)` has finite-context nearest-integer margin `1/32` for gaps `1` through `7`. Gaps `1` through `6` use the rational enclosures `gap/8 <= gap/(2π) <= 25*gap/157` from `π <= 4` and `3.14 < π`; gap `7` is certified in integer cell `1` by `10/9 <= 7/(2π) <= 175/157`, using `π < 3.15` for the lower side and `3.14 < π` for the upper side. This is a real standard-RoPE theorem, but only for context `8`; it is not a proof for 512, 4096, 128k, or the whole multi-channel bank.
