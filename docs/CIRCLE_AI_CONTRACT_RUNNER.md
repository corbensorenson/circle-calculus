...
containing request preflight, receipt, and gate report.
...
submitted request object used for preflight. The embedded receipt still carries
its own `request_content_fingerprint` for the canonical request that the
certifier emitted after applying contract defaults.
