...
def test_standalone_batch_artifact_verifier_rejects_stale_runner_gate_policy(
    tmp_path: Path,
) -> None:
    runner_check_path = _emit_rope_model_only_architecture_batch_artifacts(tmp_path)
    runner_check = json.loads(runner_check_path.read_text(encoding="utf-8"))
    runner_check["gate_policy"].pop("require_no_unsupported_architecture_fields")
    ...
    assert rejection["runner_gate_policy"][
        "require_no_unsupported_architecture_fields"
    ] is False
    assert any(
        "require_no_unsupported_architecture_fields must be a boolean" in failure
...
def test_standalone_batch_artifact_verifier_rejects_missing_runner_required_kind(
    ...
    runner_check_path = _emit_rope_model_only_architecture_batch_artifacts(tmp_path)
    runner_check = json.loads(runner_check_path.read_text(encoding="utf-8"))
    runner_check["required_kinds"] = ["kv_cache_ring_buffer"]
    runner_check_path.write_text(
        json.dumps(runner_check, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    ...
    rejection = json.loads(result.stderr)
    assert rejection["accepted"] is False
    assert rejection["runner_required_kinds"] == ["kv_cache_ring_buffer"]
    assert rejection["kind_counts"] == {"rope_position_distinguishability": 1}
    assert any(
        "runner-check required kind is missing: kv_cache_ring_buffer" in failure
        for failure in rejection["failures"]
    )
...
def test_standalone_batch_artifact_verifier_rejects_stale_runner_proof_status(
    tmp_path: Path,
) -> None:
    runner_check_path = _emit_rope_model_only_architecture_batch_artifacts(tmp_path)
    runner_check = json.loads(runner_check_path.read_text(encoding="utf-8"))
    runner_check["summaries"][0]["all_theorem_ids_proved"] = False
    runner_check["summaries"][0]["unproved_theorem_ids"] = ["STALE-T0000"]
    runner_check_path.write_text(
        json.dumps(runner_check, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    ...
    assert rejection["accepted"] is False
    assert any(
        "summary all_theorem_ids_proved does not match receipt" in failure
        for failure in rejection["failures"]
    )
    assert any(
        "summary unproved_theorem_ids does not match receipt" in failure
        for failure in rejection["failures"]
    )
...
def test_standalone_batch_artifact_verifier_rejects_unsupported_architecture_fields_when_required(
    ...
    rejection = json.loads(result.stderr)
    assert rejection["accepted"] is False
    assert rejection["require_no_unsupported_architecture_fields"] is True
    assert rejection["failure_count"] == 1
    assert rejection["summaries"][0][
