...
- a finite family of admitted coil strides has exact reach/miss conditions, local-window thresholds, complete-coverage iff no positive uncovered-lag witness, raw and clipped candidate-budget bounds, theorem-side candidate-list semantics, no-duplicate budget-survival criteria, finite positive-lag coverage iff, finite covered/uncovered-list iffs/counts, concrete default-plan hit/gap witnesses, and a compact complete-coverage PASS fixture (`AIT-T0015` through `AIT-T0091`).
...
The real-phase RoPE program is separate from that exact integer-period contract. It now has theorem-backed finite-margin seeds, including the rational/discretized `1/4099` context-4096 certificate, generated-gap and floor/ceiling nearest-integer bridges, interval-certificate and band-endpoint bridges, and genuine standard channel-0 interval seeds for `1 / (2*pi)` through context `16384` at margin `1/104219`. The D12, D13, and D14 bridge theorems package conditional one-separating-channel bank consequences when a bank contains standard channel 0. The bracket theorems package the channel-0 reading as: `1/104219` is proved at 4k, 8k, and 16k, while any margin at or above `1/104218` is impossible once gap `710` is in context. The sidecar's `standard_channel0_frontier_summary` and `standard_band_certificate_audits` record 32k and 64k exact-rational candidate plans that pass the executable band audit, plus a larger 128k request whose first uncovered gap is `103993`; those frontier rows are not Lean-proved. `AIRA-T0139` through `AIRA-T0141` now prove the rational-band compression and endpoint-reflection route needed to convert generated band lists without another large per-gap interval-case expansion. These are one-channel or conditional bank-level real-phase certificates plus certificate-generation infrastructure, not a full 128k standard-RoPE bank theorem.
