...
The first command uses looped-transformer aliases and emits the same canonical
receipt fields as `--loop-period`, `--sample-index`, `--max-loops`,
`--token-count`, `--selected-block-start`, `--selected-block-width`, and
`--shift-passes`. `--shift-amount` must be a whole multiple of the period, so
the whole-period index-reuse theorem remains the checked object.
