...
The score trace has its own formal contract. A step is active exactly when it lies between the required loop depth and the overthinking boundary. The first active trace step is the required loop depth, and a budgeted exit exists exactly when that first active step is inside `maxLoops`.
