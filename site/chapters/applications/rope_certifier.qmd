...
For genuine standard channel 0, the Lean-proved margin currently reaches context `196608` at margin `1/328459`. The exact-rational planner and generated Lean certificate route still prove the sharper 32k and 64k rows at margin `1/104219`; the stronger 128k request at that same margin fails at gap `103993`. The important user-facing result is the D19 classifier: at context `131072`, margin `1/328459` is proved, margin `1/328458` is impossible, and the tiny open interval between those thresholds remains deliberately undecided.
...
The public Circle AI contract pack exports two D19 classifier probes at context `131072`: margin `1/328459` has status `proved`, while margin `1/328458` has status `impossible`. Those fields give downstream tools a theorem-backed frontier check without requiring them to import Lean.
...
`ROPE-AUDIT-EXACT-INTEGER-PHASE-BANK` points a consumer to the declared integer-period phase-bank collision audit. `ROPE-USE-D19-MARGIN-FRONTIER` points a consumer to the D19 standard-channel-0 margin frontier. They are planner/audit records over theorem-backed fields, not claims that a real model gets longer context, lower loss, or full all-channel real-RoPE guarantees.
...
D19 is the current proved standard-channel frontier. The D19 bridge turns that channel-0 seed into a conditional one-separating-channel bank certificate when standard channel 0 is present: `1/328459` is proved for every requested context with `103993 < context <= 196608`, while every margin at or above `1/328458` is impossible throughout that range. This is still not a full all-channel margin theorem.
