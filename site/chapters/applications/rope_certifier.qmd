...
- The per-channel single-period counts are exact for each individual integer period; they are not all-channel bank counts. `AIRA-T0203` proves the zero-count interpretation for one positive period: zero means no unequal in-context collision in that one channel.
