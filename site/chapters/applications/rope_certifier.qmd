...
- For the diagnostic prefix preset `[6, 9, 13, 18]` at context `128`, `AIRA-T0188` proves that the first passing prefix has length `3`, and `AIRA-T0189` proves that singleton subbanks are insufficient while `[13, 18]` is sufficient. These are named finite report certificates, not a global subfamily-minimization theorem.
