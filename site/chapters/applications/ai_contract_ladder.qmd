...
The conditional bank bridge says that if a finite real-phase bank contains that channel and the inspected context and margin are within the seed, then the all-channel near-turn condition is ruled out under the stated tolerance scaling. The theorem trail also proves a useful negative companion: gap `710` blocks the doubled D9 margin `1/65536`, so the seed cannot be strengthened by only changing the advertised margin.
