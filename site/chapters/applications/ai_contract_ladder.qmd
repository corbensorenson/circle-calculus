...
2. **KV-cache ring buffer:** add the time boundary that distinguishes a live residue from a stale same-slot token, then add the sink-window policy that pins only the seen prefix before the rolling window.
3. **Sparse-attention coverage:** inspect a plan that intentionally has both covered lags and theorem-backed gap witnesses.

After those three, the cyclic-memory, multicoil phase-feature, recurrence, and
mixer lessons should read as variations on the same pattern: identify the
finite circular object, name the repeated action, expose a failure mode, and
keep empirical model claims separate.

## Run The Ladder

The fastest way to understand the lane is to run one certificate from each
major family, then inspect the generated theorem ids and non-claims:
...
python scripts/circle_ai_contract_ready.py --action-plan --format json
```
...
[looped recurrence contracts](looped_recurrence_contracts.qmd), and
[circulant/block-cyclic mixers](circulant_block_cyclic_mixer.qmd).
...
<p>Full theorem audit: <a href="ai_contract_ladder_audit.qmd">AI Contract Ladder Proof Audit</a> and <a href="rope_certifier_audit.qmd">RoPE Proof Audit</a>.</p>
...
For the narrower candidate-budget slice, continue with the
[strided candidate fanout lesson](strided_candidate_fanout.qmd).
...
[Open the focused circulant and block-cyclic mixer lesson](circulant_block_cyclic_mixer.qmd).

<div class="theorem-box" data-theorem-id="AIT-T0006"></div>

<p>Full theorem-card trail for this ladder: <a href="ai_contract_ladder_audit.qmd">AI Contract Ladder Proof Audit</a>.</p>
