...
For KV-cache, sparse attention, recurrence, strided-fanout, cyclic-memory,
multicoil-phase, cyclic-mixer, seed-rule, and already-normalized RoPE requests,
use `--request-file` with a
...
`--format compact-json` or `--compact-json-out` when another project only needs
