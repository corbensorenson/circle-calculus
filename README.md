...
The certifier emits theorem ids, Lean declaration names, a finite slot/window certificate, retained-batch distinctness, a no-same-slot-overwrite-before-current read guard, an explicit stale-token same-slot overwrite witness, single-token and batch retained-iff-no-later-same-slot-write trace contracts, next-overwrite boundary iff fields, a modeled adapter request-trace pass/fail certificate, a pass-iff-boundary checklist, trace-fresh read-batch slot distinctness, a live-window slot-count iff, a full generated live-window coverage flag, and a full-coverage-contract iff. It proves finite ring-buffer indexing and overwrite-window facts only; it is not a paging-policy, throughput, memory-saving, retrieval-quality, implementation-correctness, deployment-safety, or model-quality claim.
