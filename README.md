...
| `papers/physics/PAPER_PHYS_01_PROOF_CARRYING_LATTICE_GAUGE.md` | draft/proved finite seed | `PHYS-T0001` through `PHYS-T0046` are Lean-proved finite `ZMod n` phase-list and link-path holonomy, source-target composability, singleton-concat/triple/four-link, empty-identity, concat-identity, concat-associativity, reversal-algebra, boundary-append composability, source/target projection, checked path identity/associativity/holonomy interface, checked-to-link-path projection bridges, checked closed-loop and two-path cycle gauge-invariance facts, gauge-transform, Wilson-loop, and plaquette invariance facts with Python examples |
