...
| `papers/physics/PAPER_PHYS_01_PROOF_CARRYING_LATTICE_GAUGE.md` | draft/proved finite seed | `PHYS-T0001` through `PHYS-T0049` are Lean-proved finite `ZMod n` phase-list and link-path holonomy, source-target composability, singleton-concat/triple/four-link, empty-identity, concat-identity, concat-associativity, reversal-algebra, boundary-append composability, source/target projection, checked path identity/associativity/holonomy interface, checked-to-link-path projection bridges, checked closed-loop and two-path cycle gauge-invariance facts, closed-loop record identity/cycle holonomy facts, gauge-transform, Wilson-loop, and plaquette invariance facts with Python examples |
| `papers/generative/PAPER_GEN_01_SEED_RULE_PROVENANCE.md` | draft/proved finite seed | `GEN-T0001` through `GEN-T0027` have checked Lean declarations for finite seed-rule, coil-orbit, representative-indexed orbit-decomposition, orbit-count/period/coverage/orbit-class agreement, canonical representative coverage/disjointness, proof-glyph, exact-comparison regeneration seeds, exact-regeneration self/symmetry/transitivity facts, comparison field-equality/negative-case gates, generated-list length certificates, empty declared-search boundary facts, and bounded best-candidate soundness facts; Python fixtures now include bounded finite-list generator search |
