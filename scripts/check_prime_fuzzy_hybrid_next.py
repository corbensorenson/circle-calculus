...
DEFAULT_CSV = (
    ...
    / "PRIME_ENGINE"
    / "results"
    / "prime_fuzzy_hybrid_next_latest.csv"
)
DEFAULT_METADATA = (
    ROOT
    / "sidecars"
    / "PRIME_ENGINE"
    / "results"
    / "prime_fuzzy_hybrid_next_latest.json"
)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate the fuzzy hybrid exact-next benchmark artifact."
    )
    ...
    if failures:
        ...
        return 1
    print(
        "prime fuzzy hybrid next artifact ok: "
...
def validate_artifact(
    *,
    ...
    failures.extend(validate_timing_rows(timing_rows, starts))
...
def validate_timing_rows(timing_rows: list[TimingRow], starts: set[int]) -> list[str]:
    failures = []
    ...
    for start, rows in timing_by_start.items():
        ...
        if len(results) != 1:
            failures.append(
                f"start {start} timing rows disagree on result: {sorted(results)}"
            )
        for row in rows:
...
def validate_speedup_rows(speedup_rows: list[SpeedupRow], starts: set[int]) -> list[str]:
    ...
    for start in starts:
        rows = [
            ...
            if row.start == start
            and row.name == "circle_fuzzy_hybrid_rust_server_exact_next"
        ]
