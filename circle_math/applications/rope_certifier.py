...
ROPE_CERTIFIER_LEAN_DECLARATIONS: tuple[str, ...] = (
    ...
    "Circle.Applications.ropePeriodBankLCM_pos_of_forall_mem_pos",
    "Circle.Applications.ropePhaseBankNoCollisionOnContext_iff_lcm_ge_context_of_forall_mem_pos",
    "Circle.Applications.ropePhaseBankNoCollisionOnContext_iff_lcmCollisionPairCount_eq_zero_of_forall_mem_pos",
    ...
    "Circle.Applications.ropeCollisionPairCountAtGapMultiples_eq_fittingRange",
    "Circle.Applications.ropeCollisionPairCountAtGapMultiples_twice_eq_closedForm",
    "Circle.Applications.ropeCollisionPairCountAtGapMultiples_eq_closedForm",
)
...
DIAGNOSTIC_COUNT_THEOREM_IDS: dict[tuple[int, tuple[int, ...]], str] = {
    (64, (6, 18, 54)): "AIRA-T0198",
    (256, (32, 48, 96)): "AIRA-T0199",
    (241, (15, 16)): "AIRA-T0200",
    (960, (60, 64)): "AIRA-T0201",
    (961, (60, 64)): "AIRA-T0202",
}

DIAGNOSTIC_COUNT_LEAN_DECLARATIONS: dict[str, str] = {
...
PHASE_BANK_SMALLEST_SUBFAMILY_THEOREMS: tuple[str, ...] = (
    "AIRA-T0193",
    "AIRA-T0194",
)


def _diagnostic_count_theorem_ids(context_length: int, periods: Sequence[int]) -> tuple[str, ...]:
    theorem_id = DIAGNOSTIC_COUNT_THEOREM_IDS.get((context_length, tuple(periods)))
    return () if theorem_id is None else (theorem_id,)


def _diagnostic_count_lean_declarations(
    context_length: int,
    periods: Sequence[int],
) -> tuple[str, ...]:
    return tuple(
        DIAGNOSTIC_COUNT_LEAN_DECLARATIONS[theorem_id]
        for theorem_id in _diagnostic_count_theorem_ids(context_length, periods)
    )
...
ROPE_REAL_PHASE_PRECURSOR_LEAN_DECLARATIONS: tuple[str, ...] = (
    ...
    "Circle.Applications.ropeTurnRatioExactWeakestGapMargin_iff_finiteMargin_and_realized",
...
ROPE_NAT_RATIO_MODULAR_INVERSE_EXACT_MARGIN_THEOREMS: tuple[str, ...] = (
    "AIRA-T0224",
    "AIRA-T0225",
    "AIRA-T0226",
)

ROPE_NAT_RATIO_MODULAR_INVERSE_EXACT_MARGIN_LEAN_DECLARATIONS: tuple[str, ...] = (
    "Circle.Applications.ropeTurnRatioGapNearestIntegerMargin_le_error",
    "Circle.Applications.ropeTurnRatioError_natRatio_eq_one_over_den_of_modular_inverse_witness",
    "Circle.Applications.ropeTurnRatioNatRatio_exactWeakestGapMargin_report_of_modular_inverse_witness",
)
...
ROPE_STANDARD_CHANNEL0_INTERVAL_SEED_NAME = "standard_rope_channel0_interval_context_196608"
...
ROPE_STANDARD_CHANNEL0_D18_BANK_BRIDGE_THEOREMS: tuple[str, ...] = (
    "AIRA-T0165",
    "AIRA-T0166",
)

ROPE_STANDARD_CHANNEL0_D18_BANK_BRIDGE_LEAN_DECLARATIONS: tuple[str, ...] = (
    "Circle.Applications.not_ropeRealPhaseBankNearTurn_of_standardChannel0D18Seed",
    "Circle.Applications.not_ropeRealPhaseBankNearTurn_of_standardChannel0D18Seed_cons",
)
...
class PhaseBankPrefixCollisionReport:
    ...
    collision_pair_count_closed_form_numerator: int
...
class ExactDiscreteRoPECertificate:
    ...
    common_gap_collision_pair_count_closed_form_numerator: int
...
@dataclass(frozen=True)
class StandardChannel0D18BankBridgeCertificate:
    schema_id: str
    name: str
    requested_context: int
    requested_margin: str
...
@dataclass(frozen=True)
class StandardChannel0D18MarginBracketCertificate:
    schema_id: str
    name: str
    context_length: int
    proved_margin: str
...
@dataclass(frozen=True)
class StandardChannel0D19BankBridgeCertificate:
    schema_id: str
    name: str
    requested_context: int
    requested_margin: str
...
@dataclass(frozen=True)
class StandardChannel0D19MarginBracketCertificate:
    schema_id: str
    name: str
    context_length: int
...
def collision_pair_count_at_gap_multiples_fitting_range(context_length: int, common_gap: int) -> int:
    """Count guaranteed starts over the quotient-bounded fitting multiple range."""
    if common_gap <= 0 or common_gap >= context_length:
        return 0
    fitting_count = collision_pair_count_fitting_multiple_count(context_length, common_gap)
    return sum(
        collision_pair_count_at_gap(context_length, multiple * common_gap)
        for multiple in range(1, fitting_count + 1)
    )


def collision_pair_count_at_gap_multiples_closed_form_numerator(
    context_length: int,
    common_gap: int,
) -> int:
    """Return the checked numerator for twice the positive-multiple pair count."""
    if common_gap <= 0 or common_gap >= context_length:
        return 0
    fitting_count = collision_pair_count_fitting_multiple_count(context_length, common_gap)
    return fitting_count * (2 * context_length - common_gap * (fitting_count + 1))


def collision_pair_count_at_gap_multiples_closed_form(context_length: int, common_gap: int) -> int:
    """Return the exact closed-form positive-multiple pair count."""
    return collision_pair_count_at_gap_multiples_closed_form_numerator(
        context_length,
        common_gap,
    ) // 2
...
def collision_pair_count_at_gap_multiples(context_length: int, common_gap: int) -> int:
    ...
    return collision_pair_count_at_gap_multiples_closed_form(context_length, common_gap)
...
def single_gap_boundary_collision_pair_count(context_length: int, common_gap: int) -> int | None:
    ...
    Returns ``context_length - common_gap`` exactly when the Lean theorem
    hypotheses hold: ``0 < common_gap < context_length <= 2 * common_gap``.
    Outside that range, later multiples may contribute or no repeat fits, so
    this helper intentionally declines to give a formula value.
    """
    if 0 < common_gap < context_length <= 2 * common_gap:
        return context_length - common_gap
    return None
...
def _subfamily_report_theorem_ids(
    ...
    if (
        ...
        theorem_ids.extend(PHASE_BANK_SMALLEST_SUBFAMILY_THEOREMS)
        theorem_ids.append("AIRA-T0189")
        theorem_ids.append("AIRA-T0195")
    return tuple(theorem_ids)
...
def phase_bank_prefix_collision_reports(
    ...
    *,
    ...
    for prefix_length in range(1, min(len(normalized_periods), limit) + 1):
        ...
        closed_form_numerator = 0 if reaches_context else (
        ...
        reports.append(PhaseBankPrefixCollisionReport(
            ...
            collision_pair_count_closed_form_numerator=closed_form_numerator,
...
def turn_ratio_nat_ratio_modular_inverse_margin_witness(
    *,
    numerator: int,
    denominator: int,
    gap: int,
) -> tuple[int, str] | None:
    """Return a ``+1`` or ``-1`` modular-inverse witness for one rational gap.

    Lean proves that either witness shape gives exact nearest-integer error
    ``1 / denominator`` for ``gap * numerator / denominator``. The returned
    integer is the nearest turn used by the theorem.
    """
    if numerator < 0:
        raise ValueError("numerator must be nonnegative")
    if denominator <= 0:
    ...
    product = gap * numerator
    if product % denominator == 1:
        return (product - 1) // denominator, "plus_one"
    if product % denominator == denominator - 1:
        return (product + 1) // denominator, "minus_one"
    return None
    ...
    *,
    ...
    if certificate_name == ROPE_RATIONAL_PRESET_4099_NAME and pass_certificate:
        ...
        if numerator == 1 and 1 < context_length <= denominator:
            ...
            theorem_ids = (
                theorem_ids + ROPE_NAT_RATIO_MODULAR_INVERSE_EXACT_MARGIN_THEOREMS
            )
            lean_declarations = (
                lean_declarations
...
def plan_standard_channel0_interval_bands(
    *,
    ...
    proven_d20_160k_d18_seed = (
        pi_bound_preset == "d20"
        and margin == Fraction(1, 328459)
        and context_length == 163840
        and first_uncovered_gap is None
    )
    proven_d20_192k_d19_seed = (
        pi_bound_preset == "d20"
        and margin == Fraction(1, 328459)
        and context_length == 196608
        and first_uncovered_gap is None
    )
    return StandardRoPEIntervalPlan(
        ...
        theorem_status=(
            ...
            else "lean_proved_interval_seed_AIRA-T0162_to_AIRA-T0164"
            if proven_d20_160k_d18_seed
            else "lean_proved_interval_seed_AIRA-T0168_to_AIRA-T0170"
            if proven_d20_192k_d19_seed
        ...
        explanation=(
            ...
            "context-65536 plans plus the margin-1/328459 context-131072 and "
            "context-163840/context-196608 plans have now "
...
def certify_standard_channel0_interval_seed() -> IntervalBackedTurnRatioCertificate:
    ...
    turn ratio ``1 / (2π)`` over the context containing gaps 1 through 196607.
    ...
    theorem-backed channel-0 seed through context 131072; D18 keeps that
    margin and extends the compiled seed through context 163840; D19 extends
    the same margin through context 196608.
    """
    ...
    return IntervalBackedTurnRatioCertificate(
        ...
        explanation=(
            ...
            "has finite nearest-integer margin 1/328459 for gaps 1 through 196607. "
            "The generated D19 compressed certificate uses the 20-decimal enclosure "
            "10^20*gap/628318530717958647694 <= gap/(2*pi) <= "
            "10^20*gap/628318530717958647692, split across computed integer "
            "cells 0 through 31290. Lean also proves that the earlier 1/1024 "
            ...
            "margin to 1/328459 for the 128k context, and D18 carries that same "
            "margin to the 160k context. D19 carries it to the 192k context; "
            "the adjacent larger "
            "margin 1/328458 is impossible once gap 103993 is in scope. "
            "D16, D17, D18, and D19 package their corresponding brackets and add "
        ...
        claim_boundary=(
            ...
            "RoPE channel-0 turn ratio over context 196608, plus conditional "
...
def certify_standard_channel0_d18_bank_request(
    *,
    requested_context: int,
    requested_margin: Fraction = Fraction(1, 328459),
    first_channel_shape: bool = True,
) -> StandardChannel0D18BankBridgeCertificate:
    """Certify whether a request is inside the D18 standard-channel bank bridge."""
    if requested_context <= 0:
        raise ValueError("requested_context must be positive")
    if requested_margin < 0:
        raise ValueError("requested_margin must be nonnegative")

    certified_context = 163840
    certified_margin = Fraction(1, 328459)
    context_ok = requested_context <= certified_context
    margin_ok = requested_margin <= certified_margin
    pass_certificate = context_ok and margin_ok
    if not context_ok:
        failure_reason = "requested_context_exceeds_d18_seed"
    elif not margin_ok:
        failure_reason = "requested_margin_exceeds_d18_seed"
    else:
        failure_reason = None

    if first_channel_shape:
        theorem_ids = ROPE_STANDARD_CHANNEL0_D18_BANK_BRIDGE_THEOREMS
        lean_declarations = ROPE_STANDARD_CHANNEL0_D18_BANK_BRIDGE_LEAN_DECLARATIONS
        bank_shape = "standard_channel0_first"
        assumptions = (
            "The finite real-phase bank has standard channel 0 as its first channel.",
            "The requested context is at most 163840.",
            "The requested margin is at most 1/328459.",
        )
    else:
        theorem_ids = ("AIRA-T0165",)
        lean_declarations = (
            "Circle.Applications.not_ropeRealPhaseBankNearTurn_of_standardChannel0D18Seed",
        )
        bank_shape = "contains_standard_channel0"
    ...
    return StandardChannel0D18BankBridgeCertificate(
        schema_id="circle_calculus.standard_rope_channel0_d18_bank_bridge.v0",
        name="standard_rope_channel0_d18_bank_bridge_request",
        requested_context=requested_context,
        requested_margin=format_fraction(requested_margin),
        certified_context=certified_context,
        certified_margin=format_fraction(certified_margin),
        ...
        tolerance_rule="Lean conclusion applies when tolerance < fullTurn * requestedMargin.",
        explanation=(
            "This request is inside the D18 standard-channel bank bridge."
            if pass_certificate
            else "This request is outside the D18 standard-channel bank bridge."
        ),
        claim_boundary=(
            "This is a conditional one-separating-channel bank certificate based on "
            "standard channel 0. It is not a full all-channel standard-RoPE margin "
...
def certify_standard_channel0_d18_margin_bracket() -> StandardChannel0D18MarginBracketCertificate:
    """Return the theorem-backed 160k standard-channel margin bracket."""

    return StandardChannel0D18MarginBracketCertificate(
        schema_id="circle_calculus.standard_rope_channel0_d18_margin_bracket.v0",
        name="standard_rope_channel0_d18_context163840_margin_bracket",
        context_length=163840,
        proved_margin="1/328459",
        impossible_margin_floor="1/328458",
        pass_certificate=True,
        theorem_ids=("AIRA-T0162", "AIRA-T0163", "AIRA-T0155", "AIRA-T0167"),
        lean_declarations=(
            "Circle.Applications.ropeStandardChannel0D18Seed_intervalCertificate",
            "Circle.Applications.ropeStandardChannel0D18Seed_turnRatioFiniteMargin",
            "Circle.Applications.not_ropeStandardChannel0_margin_ge_one_over_328458_of_context_gt_103993",
        ...
        explanation=(
            "Lean proves that standard RoPE channel 0 has finite-context "
            "nearest-integer margin 1/328459 through context 163840. Lean also "
            "proves that any margin at or above 1/328458 is impossible for "
            "that context because gap 103993 is already too close to integer turn 16551."
        ),
        claim_boundary=(
            "This is a 160k one-channel standard-RoPE bracket. It is not a full "
            "all-channel bank margin theorem, and it does not decide margins "
            "strictly between 1/328459 and 1/328458."
        ),
    )
...
def certify_standard_channel0_d19_bank_request(
    *,
    requested_context: int,
    requested_margin: Fraction = Fraction(1, 328459),
    first_channel_shape: bool = True,
) -> StandardChannel0D19BankBridgeCertificate:
    """Certify whether a request is inside the D19 standard-channel bank bridge."""
    if requested_context <= 0:
        raise ValueError("requested_context must be positive")
    if requested_margin < 0:
        raise ValueError("requested_margin must be nonnegative")

    certified_context = 196608
    certified_margin = Fraction(1, 328459)
    context_ok = requested_context <= certified_context
    margin_ok = requested_margin <= certified_margin
    pass_certificate = context_ok and margin_ok
    if not context_ok:
        failure_reason = "requested_context_exceeds_d19_seed"
    elif not margin_ok:
        failure_reason = "requested_margin_exceeds_d19_seed"
    else:
        failure_reason = None

    if first_channel_shape:
        theorem_ids = ROPE_STANDARD_CHANNEL0_D19_BANK_BRIDGE_THEOREMS
        lean_declarations = ROPE_STANDARD_CHANNEL0_D19_BANK_BRIDGE_LEAN_DECLARATIONS
        bank_shape = "standard_channel0_first"
        assumptions = (
            "The finite real-phase bank has standard channel 0 as its first channel.",
            "The requested context is at most 196608.",
            "The requested margin is at most 1/328459.",
        )
    else:
        theorem_ids = ("AIRA-T0171",)
        lean_declarations = (
            "Circle.Applications.not_ropeRealPhaseBankNearTurn_of_standardChannel0D19Seed",
        )
        bank_shape = "contains_standard_channel0"
    ...
    return StandardChannel0D19BankBridgeCertificate(
        schema_id="circle_calculus.standard_rope_channel0_d19_bank_bridge.v0",
        name="standard_rope_channel0_d19_bank_bridge_request",
        requested_context=requested_context,
        requested_margin=format_fraction(requested_margin),
        certified_context=certified_context,
        certified_margin=format_fraction(certified_margin),
        ...
        tolerance_rule="Lean conclusion applies when tolerance < fullTurn * requestedMargin.",
        explanation=(
            "This request is inside the D19 standard-channel bank bridge."
            if pass_certificate
            else "This request is outside the D19 standard-channel bank bridge."
        ),
        claim_boundary=(
            "This is a conditional one-separating-channel bank certificate based on "
            "standard channel 0. It is not a full all-channel standard-RoPE margin "
...
def certify_standard_channel0_d19_margin_bracket() -> StandardChannel0D19MarginBracketCertificate:
    """Return the theorem-backed 192k standard-channel margin bracket."""

    return StandardChannel0D19MarginBracketCertificate(
...
def certify_rope_positions(config: RoPEConfig) -> RoPEPositionCertificate:
    ...
    proof_layers = (
        ...
        RoPEProofLayerReport(
            ...
            explanation=(
                "The bracketed 192k genuine standard-RoPE interval seed certifies channel 0 "
                "with turn ratio 1/(2*pi), margin 1/328459, and context 196608; "
                "the same theorem trail refutes every advertised margin at or above "
                "1/328458 for that channel and context, and D19 one-separating-channel bank bridges "
    ...
    return RoPEPositionCertificate(
        ...
        lean_declarations=exact.lean_declarations,
    )
...
def certify_exact_discrete_phase_bank(
    ...
    *,
    ...
    diagnostic_count_theorem_ids = _diagnostic_count_theorem_ids(
        context_length,
        discrete_periods,
    )
    diagnostic_count_lean_declarations = _diagnostic_count_lean_declarations(
        context_length,
        discrete_periods,
    )
    return ExactDiscreteRoPECertificate(
        pass_exact=exact_pass,
        theorem_ids=ROPE_CERTIFIER_THEOREMS + diagnostic_count_theorem_ids,
        lean_declarations=ROPE_CERTIFIER_LEAN_DECLARATIONS + diagnostic_count_lean_declarations,
        ...
        common_gap_collision_pair_count_closed_form_numerator=closed_form_numerator,
        ...
        assumptions=(
            ...
            f"Each channel period is a positive integer {period_source}; AIRA-T0179 proves the resulting period-bank LCM is positive.",
            ...
            "Lean theorem AIRA-T0211 proves the positive-multiple count equals the finite sum over that quotient-bounded fitting range, so the executable loop does not depend on scanning noncontributing multiples.",
            "Lean theorem AIRA-T0212 proves the doubled closed form: 2 * total_bank_collision_pair_count equals common_gap_fitting_multiple_count * (2 * context_length - common_gap * (common_gap_fitting_multiple_count + 1)) for the declared positive common gap.",
            "Lean theorem AIRA-T0213 divides that checked numerator by two, proving the exact closed-form value used by the executable total-count helper.",
...
def certificate_summary_lines(certificate: RoPEPositionCertificate) -> tuple[str, ...]:
    ...
    return (
        ...
        f"common_gap_collision_pair_count_closed_form_numerator="
        f"{exact.common_gap_collision_pair_count_closed_form_numerator} "
